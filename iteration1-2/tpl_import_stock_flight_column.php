...
/**
 * Helper function to preview stock flight column import via API
 * 
 ...
 */
function previewStockFlightColumnViaAPI($api_url, $csv_data) {
    // Validate inputs
    if (empty($api_url)) {
        error_log("[Stock Flight Import] Error: API URL is empty");
        return ['error' => 'API URL is empty', 'debug' => 'api_url_empty'];
    }
    
    if (empty($csv_data) || !is_array($csv_data)) {
        error_log("[Stock Flight Import] Error: CSV data is empty or not an array");
    ...
    if ($jsonData === false) {
        ...
        error_log("[Stock Flight Import] JSON encode error: $jsonError");
        return ['error' => 'Failed to encode request data', 'debug' => 'json_encode_failed', 'detail' => $jsonError];
    }
    
    $apiEndpoint = $api_url . '/stock/flight-column/preview';
    error_log("[Stock Flight Import] Calling API: $apiEndpoint");
    error_log("[Stock Flight Import] Request data size: " . strlen($jsonData) . " bytes, CSV rows: " . count($csv_data));
    
    $ch = curl_init($apiEndpoint);
    if ($ch === false) {
        error_log("[Stock Flight Import] cURL init failed");
    ...
    if ($curlError || $curlErrno) {
        error_log("[Stock Flight Import] cURL error #$curlErrno: $curlError");
        return ['error' => 'cURL request failed', 'debug' => 'curl_error', 'detail' => $curlError, 'errno' => $curlErrno];
    }
    
    if ($httpCode !== 200 && $httpCode !== 201) {
        error_log("[Stock Flight Import] API call failed. HTTP Code: $httpCode");
        error_log("[Stock Flight Import] Response: " . substr($response, 0, 500));
        return ['error' => 'API returned error status', 'debug' => 'http_error', 'http_code' => $httpCode, 'response' => substr($response, 0, 500)];
    }
    
    if (empty($response)) {
        error_log("[Stock Flight Import] API returned empty response");
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        $jsonError = json_last_error_msg();
        error_log("[Stock Flight Import] JSON decode error: $jsonError");
        error_log("[Stock Flight Import] Response preview: " . substr($response, 0, 200));
    ...
    if (isset($responseData['status']) && $responseData['status'] === 'success' && isset($responseData['data'])) {
        ...
        error_log("[Stock Flight Import] API response structure: status=" . $responseData['status'] . ", data exists");
        return $responseData['data'];
    }
    
    // Fallback: check for direct success field (old format)
    if (isset($responseData['success'])) {
        error_log("[Stock Flight Import] API response has direct success field");
        return $responseData;
    }
    
    // If neither structure matches, log and return error
    error_log("[Stock Flight Import] API response format unexpected");
    error_log("[Stock Flight Import] Response structure: " . print_r(array_keys($responseData), true));
    error_log("[Stock Flight Import] Full response: " . print_r($responseData, true));
...
/**
 * Helper function to import stock flight column via API
 * 
 * @param string $api_url Base API URL
 ...
 */
function importStockFlightColumnViaAPI($api_url, $records, $updated_by) {
    $requestData = [
        'records' => $records,
        'updated_by' => $updated_by
    ];
    
    $ch = curl_init($api_url . '/stock/flight-column/import');
    curl_setopt_array($ch, [
        CURLOPT_POST => true,
    ...
    if ($curlError) {
        error_log("[Stock Flight Import] Import cURL error: $curlError");
        return false;
    }
    ...
    if ($httpCode === 200 || $httpCode === 201) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            ...
            error_log("[Stock Flight Import] Import JSON decode error: " . json_last_error_msg());
            return false;
        }
    } else {
        error_log("[Stock Flight Import] Import API call failed. HTTP Code: $httpCode, Response: $response");
        return false;
    }
}
    		...
    		if (isset($_POST["import_pricing"])) 
    			{
    				...
    				if (!isset($_FILES["file"]) || $_FILES["file"]["error"] !== UPLOAD_ERR_OK) {
    					...
    					echo '</center>';
    					error_log("[Stock Flight Import] File upload error: $uploadError");
    				}
    				elseif ($_FILES["file"]["size"] > 0) 
    				{
    					...
    					if (!file_exists($fileName)) {
    						...
    						echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    						echo '</center>';
    						error_log("[Stock Flight Import] File not found: $fileName");
    					}
    					elseif (!is_readable($fileName)) {
    						...
    						echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    						echo '</center>';
    						error_log("[Stock Flight Import] File not readable: $fileName");
    					}
    					else
    					{
    						...
    						if ($file === false) {
    							...
    							echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    							echo '</center>';
    							error_log("[Stock Flight Import] Failed to open file: $fileName");
    						}
    						else
    						{
    							...
    							error_log("[Stock Flight Import] CSV file read successfully. Rows: $rowCount");
    							...
    							if (empty($csv_data)) {
    								...
    								echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    								echo '</center>';
    								error_log("[Stock Flight Import] CSV data is empty");
    							}
    							else
    							{
    								// Call API to preview
    								$result = previewStockFlightColumnViaAPI($api_url, $csv_data);
    									...
    									if (empty($preview)) {
    										...
    										foreach ($preview as $item) {
    											...
    											$tablestirng .= "<tr>
    												...
    												<td>" . htmlspecialchars($flight_2) . "</td>
    												<td>
    													<input type='hidden' name='" . htmlspecialchars($auto_id) . "_matchmaker' value='" . htmlspecialchars($match_hidden) . "'>
    													" . $match . "
    												</td>
    												<td>
    													<input type='checkbox' id='chk" . htmlspecialchars($auto_id) . "' name='" . htmlspecialchars($auto_id) . "_checkoption' value='" . htmlspecialchars($auto_id) . "@#" . htmlspecialchars($flight_2) . "@#" . htmlspecialchars($match_hidden) . "' " . $checked . " />
    												</td>
    											</tr>";
    										}
    			...
    			if (isset($_POST["submit_flight"])) 
    			{
    				...
    				if (!empty($records)) {
    					// Call API to import
    					$result = importStockFlightColumnViaAPI($api_url, $records, $currnt_userlogn);
