...
<?php
/**
 * Template Name: Date Change Request
...
if (!empty($from_date) || !empty($to_date)) {
    ...
    $show_kpis = !empty($processed_requests);
    $show_table = !empty($processed_requests) && ($current_filter !== '' || (!empty($from_date) || !empty($to_date)));
    
    // Apply filters if we have data and a filter is set
    if (!empty($processed_requests) && $current_filter) {
        switch ($current_filter) {
            case 'total_date_changes':
                $filtered_data = array_filter($processed_requests, function ($request) {
                    return $request['request_type'] === 'datechange';
                });
                break;
            case 'status_success':
                $filtered_data = array_filter($processed_requests, function ($request) {
            ...
            case 'status_in_progress':
                ...
                break;
            case 'total_cost_given':
                usort($filtered_data, function ($a, $b) {
                    return floatval($b['cost_given']) <=> floatval($a['cost_given']);
                });
                break;
            case 'total_revenue':
                usort($filtered_data, function ($a, $b) {
                    return floatval($b['total_revenue']) <=> floatval($a['total_revenue']);
                });
                break;
        }
    }
...
<!-- Date Range Filter Form at the Top -->
...
.page-btn { width: 36px; height: 36px; border-radius: 4px; border: 1px solid #ddd; background-color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
<script>
    ...
    const dateOptions = {
        ...
        onOpen: function(selectedDates, dateStr, instance) {
            // Remove placeholder when calendar opens
    ...
    if (dateFilterForm) {
        ...
        function renderTable() {
            ...
            const paginatedRows = Array.from(rows).filter(row => row.style.display !== 'none');
            
            // Hide all rows first
            rows.forEach(row => row.style.display = 'none');
            
            // Show only rows for current page
            paginatedRows.slice(start, end).forEach(row => row.style.display = '');
            
            // Update pagination controls
            updatePaginationControls(paginatedRows.length);
        }
        ...
        function applyFilters() {
            ...
            rows.forEach(row => {
                ...
                if (airlineFilter && airline !== airlineFilter) {
                    showRow = false;
                }
