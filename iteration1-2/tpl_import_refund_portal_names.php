<?php
/**
 * Template Name: Import Refund Portal Names
...
/**
 * Helper function to preview refund portal names import via API
 * 
 ...
 */
function previewRefundPortalNamesViaAPI($api_url, $csv_data) {
    // Validate inputs
    if (empty($api_url)) {
        error_log("[Refund Portal Import] Error: API URL is empty");
        return ['error' => 'API URL is empty', 'debug' => 'api_url_empty'];
    }
    
    if (empty($csv_data) || !is_array($csv_data)) {
        error_log("[Refund Portal Import] Error: CSV data is empty or not an array");
    ...
    if ($jsonData === false) {
        ...
        error_log("[Refund Portal Import] JSON encode error: $jsonError");
        return ['error' => 'Failed to encode request data', 'debug' => 'json_encode_failed', 'detail' => $jsonError];
    }
    
    $apiEndpoint = $api_url . '/refund-portal-names/preview';
    error_log("[Refund Portal Import] Calling API: $apiEndpoint");
    error_log("[Refund Portal Import] Request data size: " . strlen($jsonData) . " bytes, CSV rows: " . count($csv_data));
    
    $ch = curl_init($apiEndpoint);
    if ($ch === false) {
        error_log("[Refund Portal Import] cURL init failed");
    ...
    if ($curlError || $curlErrno) {
        error_log("[Refund Portal Import] cURL error #$curlErrno: $curlError");
        return ['error' => 'cURL request failed', 'debug' => 'curl_error', 'detail' => $curlError, 'errno' => $curlErrno];
    }
    
    if ($httpCode !== 200 && $httpCode !== 201) {
        error_log("[Refund Portal Import] API call failed. HTTP Code: $httpCode");
        error_log("[Refund Portal Import] Response: " . substr($response, 0, 500));
        return ['error' => 'API returned error status', 'debug' => 'http_error', 'http_code' => $httpCode, 'response' => substr($response, 0, 500)];
    }
    
    if (empty($response)) {
        error_log("[Refund Portal Import] API returned empty response");
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        $jsonError = json_last_error_msg();
        error_log("[Refund Portal Import] JSON decode error: $jsonError");
        error_log("[Refund Portal Import] Response preview: " . substr($response, 0, 200));
    ...
    if (isset($responseData['status']) && $responseData['status'] === 'success' && isset($responseData['data'])) {
        ...
        error_log("[Refund Portal Import] API response structure: status=" . $responseData['status'] . ", data exists");
        return $responseData['data'];
    }
    
    // Fallback: check for direct success field (old format)
    if (isset($responseData['success'])) {
        error_log("[Refund Portal Import] API response has direct success field");
        return $responseData;
    }
    
    // If neither structure matches, log and return error
    error_log("[Refund Portal Import] API response format unexpected");
    error_log("[Refund Portal Import] Response structure: " . print_r(array_keys($responseData), true));
    error_log("[Refund Portal Import] Full response: " . print_r($responseData, true));
...
/**
 * Helper function to import refund portal names via API
 * 
 * @param string $api_url Base API URL
 * @param array $records Array of records to import
 * @return array|false API response data on success, false on failure
 */
function importRefundPortalNamesViaAPI($api_url, $records) {
    $requestData = [
        'records' => $records
    ];
    
    $ch = curl_init($api_url . '/refund-portal-names/import');
    curl_setopt_array($ch, [
        CURLOPT_POST => true,
    		...
    		// IMPORT PRICING START
    			if (isset($_POST["import_pricing"])) 
    			{
    				...
    				if (!isset($_FILES["file"]) || $_FILES["file"]["error"] !== UPLOAD_ERR_OK) {
    					...
    					echo '</center>';
    					error_log("[Refund Portal Import] File upload error: $uploadError");
    				}
    				elseif ($_FILES["file"]["size"] > 0) 
    				{
    					...
    					if (!file_exists($fileName)) {
    						...
    						echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    						echo '</center>';
    						error_log("[Refund Portal Import] File not found: $fileName");
    					}
    					elseif (!is_readable($fileName)) {
    						...
    						echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    						echo '</center>';
    						error_log("[Refund Portal Import] File not readable: $fileName");
    					}
    					else
    					{
    						...
    						if ($file === false) {
    							...
    							echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    							echo '</center>';
    							error_log("[Refund Portal Import] Failed to open file: $fileName");
    						}
    						else
    						{
    							...
    							error_log("[Refund Portal Import] CSV file read successfully. Rows: $rowCount");
    							...
    							if (empty($csv_data)) {
    								...
    								echo '<br><a href="?" style="padding:10px 20px; background:#0073aa; color:white; text-decoration:none; border-radius:3px;">Back to Upload</a>';
    								echo '</center>';
    								error_log("[Refund Portal Import] CSV data is empty");
    							}
    							else
    							{
    								// Call API to preview
    								$result = previewRefundPortalNamesViaAPI($api_url, $csv_data);
    									...
    									if (empty($preview)) {
    										...
    										foreach ($preview as $item) {
    											...
    											$tablestirng .= "<tr>
    												...
    												<td>
    													<input type='checkbox' id='chk" . htmlspecialchars($auto_id) . "' name='" . htmlspecialchars($auto_id) . "_checkoption' value='" . htmlspecialchars($auto_id) . "@#" . htmlspecialchars($accountname) . "@#" . htmlspecialchars($match_hidden) . "' " . $checked . " />
    												</td>
    											</tr>";
    										}
    			...
    			if (isset($_POST["submit_pricing"])) 
    			{
    				...
    				if (!empty($records)) {
    					// Call API to import
    					$result = importRefundPortalNamesViaAPI($api_url, $records);
