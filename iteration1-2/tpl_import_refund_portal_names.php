    		...
    		if (isset($_POST["import_pricing"])) 
    			{
    				...
    				if ($_FILES["file"]["size"] > 0) 
    				{
    					...
    					while (($column = fgetcsv($file, 10000, ",")) !== FALSE) 
    					{
    						...
    						{
    							$auto_id = $column[0];
    							$accountname = $column[1];

    							$sql = "SELECT * FROM wpk4_backend_travel_payment_refunds where refund_id = '$auto_id'";
    							$result = $mysqli->query($sql);
    							$row = $result->fetch_assoc();
    							
    							$auto_id_from_table = $row['refund_id'];
    								...
    								$tablestirng.= "		
    									...
    									$tablestirng.="<td><input type='checkbox' id='chk".$auto_id."' name='".$auto_id."_checkoption' value='".$auto_id."@#".$accountname."@#".$match_hidden."' ".$checked." \/></td>
    									</tr>";
    				...
    				foreach ($_POST as $post_fieldname => $post_fieldvalue) 
    				{
    					...
    					if($postname_fieldname == 'checkoption' && isset($_POST[$check_whether_its_ticked]))
    					{
    						...
    						$post_values = explode('@#', $post_value_get);
    						    ...
    						    $sql_update_status = "UPDATE wpk4_backend_travel_payment_refunds SET 
    												canc_reason='$accountname'
    												WHERE refund_id='$auto_id_from_table_post'";
