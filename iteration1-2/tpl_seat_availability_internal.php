<?php
/**
 * Template Name: Seat Availability - Internal
...
<!-- Add this CSS to style the loading popup and dimmed background -->
<style>
#loadingOverlay {
    position: fixed;
    top: 0;
...
//if(current_user_can( 'administrator' ) || current_user_can( 'shop_manager' ) || current_user_can( 'ho_operations' ) || current_user_can( 'wp_travel_reader_sales' ) || current_user_can( 'agents' ) || current_user_can( 'user_portal_editor' ))
{
if(!isset($_GET['pg']))
{
...
<tr>
    <!--
<td width='25%'>Airline <font style='color:red'>*</font>
...
$query_r = "SELECT * FROM wpk4_backend_stock_management_sheet where dep_date > '$endoftoday' order by airline_code asc";
...
<td width='25%'>Route <font style='color:red'>*</font>
<select name='routeid' required id='routeid' onChange='updatedate(this.value)' style='width:100%; padding:10px;'>
<option value=''>Select</option>
<?php
$array_airline_code = array();
$query_r = "SELECT * FROM wpk4_backend_stock_management_sheet where dep_date > '$endoftoday' order by route asc";
$result_rg = mysqli_query($mysqli, $query_r) or die(mysqli_error($mysqli));
while($row_ty = mysqli_fetch_assoc($result_rg))
{
	$array_airline_code[] = $row_ty['route'];
}
$array_airline_code = array_unique($array_airline_code);
...
<div id="selectionprice">
    <select name='saleprice' id='saleprice' style="width:100%; padding:10px;">
	    <option value='' selected>Select</option>
	</select>
</div>
</td>
...
function updateroute(val){
  ...
  if(val == '')
	...
	else
	{
		...
		xmlhttp.open("GET","<?php echo $themepath; ?>/templates/tpl_seat_availability_internal_backend.php/?airline="+val,true);
		xmlhttp.send();
		//alert("airline");
	}
}
function updateprice(val){
  ...
  if(val == '')
	{
		document.getElementById("selectionprice").innerHTML = "<select name='saleprice' id='saleprice' style='width:100%; padding:10px;'><option value='' selected>Select</option>	</select>";
	}
	else
	{
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
		  if (this.readyState == 4 && this.status == 200) {
			document.getElementById("selectionprice").innerHTML = this.responseText;
			//showresult();
		  }
		  
		};
		xmlhttp.open("GET","<?php echo $themepath; ?>/templates/tpl_seat_availability_internal_backend.php/?routeforprice="+val,true);
		xmlhttp.send();
		//alert("airline");
	}
}
...
function showresult(){
    ...
    // Show the loading popup and dim the rest of the page
