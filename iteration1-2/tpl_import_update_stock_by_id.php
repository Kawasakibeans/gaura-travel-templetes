    		...
    		if (isset($_POST["import_pricing"])) 
    			{
    				...
    				if ($_FILES["file"]["size"] > 0) 
    				{
    					...
    					while (($column = fgetcsv($file, 10000, ",")) !== FALSE) 
    					{
    						...
    						if($column[0] == 'auto_id' && $column[1] == 'old_pnr' && $column[2] == 'new_pnr')
    						{
    							// Do Nothing
    						}
    						else
    						{
    							...
    							else
    							{
    								...
    								$tablestirng.= "		
    									...
    									$tablestirng.="<td><input type='checkbox' id='chk".$auto_id."' name='".$auto_id."_checkoption' value='".$auto_id."@#".$new_pnr."@#".$old_pnr."@#".$match_hidden."' ".$checked." \/></td>
    									</tr>";
    			...
    			if (isset($_POST["submit_stock_update"])) 
    			{
    				...
    				foreach ($_POST as $post_fieldname => $post_fieldvalue) 
    				{
    					...
    					if($postname_fieldname == 'checkoption' && isset($_POST[$check_whether_its_ticked]))
    					{
    					    ...
    					    $pnr_change_history = $old_pnr . ' -> ' . $new_pnr;
    						$values = array(
    						array($auto_id_from_table_post, "stock pnr update", $pnr_change_history, $currnt_userlogn, $current_time),
    						);
                            // add history
