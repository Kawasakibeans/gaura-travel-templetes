...
$app->get('/v1/incentive-criteria/pdf-url', function (Request $request, Response $response) {
    try {
        $queryParams = $request->getQueryParams();
        $period = $queryParams['period'] ?? null;
        
        if (!$period) {
            throw new Exception('period parameter is required', 400);
        }
        
        $service = new \App\Services\IncentiveCriteriaService();
        $result = $service->getPdfUrlByPeriod($period);
        return jsonResponse($response, $result, 'success', 'PDF URL retrieved successfully');
    }
    catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
...
$app->get('/v1/incentive-criteria/daily-eligibility-garland', function (Request $request, Response $response) {
    try {
        $queryParams = $request->getQueryParams();
        $period = $queryParams['period'] ?? null;
        ...
        $service = new \App\Services\IncentiveCriteriaService();
        $result = $service->getDailyEligibilityGarlandMinPercent($period);
        return jsonResponse($response, $result, 'success', 'Daily eligibility garland min percent retrieved successfully');
    }
    catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
...
// ======================================
// VOICE AGENT WEBHOOK RECORDS
// ======================================

$app->get('/v1/voice-agent-webhooks', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\VoiceAgentWebhookRecordsService();
        $rows = $service->list($params);
        return jsonResponse($response, $rows, 'success', 'Voice agent webhook records retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/voice-agent-webhooks/{id}', function (Request $request, Response $response, array $args) {
    try {
        $id = (int)($args['id'] ?? 0);
        if ($id <= 0) {
        ...
        $service = new \App\Services\VoiceAgentWebhookRecordsService();
        $row = $service->get($id);
        return jsonResponse($response, $row, 'success', 'Voice agent webhook record retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/voice-agent-webhooks', function (Request $request, Response $response) {
    try {
        $parsedBody = $request->getParsedBody();
        $input = is_array($parsedBody) ? $parsedBody : [];
        $service = new \App\Services\VoiceAgentWebhookRecordsService();
        $result = $service->create($input);
        return jsonResponse($response, $result, 'success', 'Voice agent webhook record created successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/after-sale-productivity-report-agent-wise', function (Request $request, Response $response) {
	try {
		$params = $request->getQueryParams();
		$service = new \App\Services\AfterSaleProductivityService();
		$rows = $service->getProductivityReportAgentWise($params);
		return jsonResponse($response, $rows, 'success', 'After-sale productivity report (agent-wise) retrieved successfully');
	}
	catch (Exception $e) {
		$code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
		return errorResponse($response, $e->getMessage(), $code);
...
// AGENT INBOUND CALL - LIST FOR DISPLAY WITH FILTERS
$app->get('/v1/agent-inbound-call/list', function (Request $request, Response $response) {
	try {
		$params = $request->getQueryParams();
		$service = new \App\Services\AgentInboundCallExtrasService();
		$result = $service->getCallsForDisplay($params);
		return jsonResponse($response, $result, 'success', 'Inbound calls retrieved successfully');
	}
	catch (Exception $e) {
		$code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
		return errorResponse($response, $e->getMessage(), $code);
...
// Get after sales daily productivity summary (SSR, tickets, audits, calls, AHT)
$app->get('/v1/after-sales-call-metrics/summary', function (Request $request, Response $response) {
    try {
        $queryParams = $request->getQueryParams();
        
        $startDate = $queryParams['start_date'] ?? null;
        $endDate = $queryParams['end_date'] ?? null;
        $agent = $queryParams['agent_name'] ?? null;
        
        $service = new \App\Services\AfterSalesCallMetricsService();
        $data = $service->getDailyProductivitySummary($startDate, $endDate, $agent);
        
        return jsonResponse($response, $data, 'success', 'After sales daily productivity summary retrieved successfully');
...
$app->get('/v1/amadeus-name-update/candidates', function (Request $request, Response $response) {
    try {
        ...
        $agentName = $queryParams['agent_name'] ?? '';

        $service = new \App\Services\AmadeusNameUpdateService();
        $data = $service->getCandidates($orderId, $agentName);

        return jsonResponse($response, $data, 'success', 'Amadeus name update candidates retrieved');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// ======================================
// MALPRACTICE CALL AUDIT ENDPOINTS
// ======================================

// GET /v1/audit/malpractice-calls - List call audit records
$app->get('/v1/audit/malpractice-calls', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        
        $filters = [];
        if (!empty($params['campaign'])) {
            $filters['campaign'] = trim($params['campaign']);
        }
        if (!empty($params['agent_name'])) {
            $filters['agent_name'] = trim($params['agent_name']);
        }
        if (!empty($params['call_date'])) {
            $filters['call_date'] = trim($params['call_date']);
        }
        if (!empty($params['recording_file_no'])) {
            $filters['recording_file_no'] = trim($params['recording_file_no']);
        }
        $filters['limit'] = 40;
        
        $service = new \App\Services\MalpracticeCallAuditService();
        $data = $service->getAuditRecords($filters);
        
        return jsonResponse($response, $data, 'success', 'Call audit records retrieved successfully');
...
$app->get('/v1/audit/malpractice-calls/{id}', function (Request $request, Response $response, array $args) {
    try {
        $id = $args['id'] ?? null;
        ...
        $service = new \App\Services\MalpracticeCallAuditService();
        $data = $service->getAuditRecordById($id);
        
        return jsonResponse($response, $data, 'success', 'Call audit record retrieved successfully');
...
$app->post('/v1/audit/malpractice-calls', function (Request $request, Response $response) {
    try {
        ...
        // Get current user from WordPress if available
        global $current_user;
        $addedBy = $current_user->user_login ?? 'system';
        
        $service = new \App\Services\MalpracticeCallAuditService();
        $data = $service->createAuditRecord($input, $addedBy);
        
        return jsonResponse($response, $data, 'success', 'Call audit record created successfully', 201);
...
$app->patch('/v1/audit/malpractice-calls/{id}', function (Request $request, Response $response, array $args) {
    try {
        $id = $args['id'] ?? null;
        ...
        $service = new \App\Services\MalpracticeCallAuditService();
        $data = $service->updateAuditRecord($id, $input, $addedBy);
        
        return jsonResponse($response, $data, 'success', 'Call audit record updated successfully');
...
$app->delete('/v1/audit/malpractice-calls/{id}', function (Request $request, Response $response, array $args) {
    try {
        $id = $args['id'] ?? null;
        ...
        $service = new \App\Services\MalpracticeCallAuditService();
        $data = $service->deleteAuditRecord($id);
        
        return jsonResponse($response, $data, 'success', 'Call audit record deleted successfully');
...
// Get dates by route (optional airline) - Line 44 in template
$app->get('/v1/monthly-price-calendar-backend/dates-by-route', function (Request $request, Response $response) use ($app) {
    try {
        $queryParams = $request->getQueryParams();
        $route = $queryParams['route'] ?? '';
        $airlineCode = $queryParams['airline'] ?? '';
        ...
        $result = $service->getDatesByRoute($route, $airlineCode);
        
        return jsonResponse($response, $result, 'success', 'Dates by route retrieved');
...
$app->get('/v1/monthly-price-calendar-backend/detailed-calendar', function (Request $request, Response $response) use ($app) {
    try {
        ...
        $month = $queryParams['month'] ?? '';
        $airlineCode = $queryParams['airline'] ?? '';
        $requireStockFromAvailability = isset($queryParams['require_stock_from_availability']) && $queryParams['require_stock_from_availability'] === '1';
        ...
        $result = $service->getDetailedCalendarData($route, $month, $airlineCode, $requireStockFromAvailability);
...
// Get comprehensive calendar data for both outbound and return routes (for showresults)
$app->get('/v1/monthly-price-calendar-backend/comprehensive-calendar', function (Request $request, Response $response) use ($app) {
    try {
        $queryParams = $request->getQueryParams();
        ...
        $result = $service->getComprehensiveCalendarData($queryParams);
        
        return jsonResponse($response, $result, 'success', 'Comprehensive calendar data retrieved');
...
$app->get('/v1/email-history/check', function (Request $request, Response $response) use ($app) {
    try {
        $params = $request->getQueryParams();
        
        $orderId = $params['order_id'] ?? null;
        $emailType = $params['email_type'] ?? null;
        ...
        $result = $service->checkEmailHistory($orderId, $emailType, $date);
        
        return jsonResponse($response, $result, 'success', 'Email history check completed');
...
$app->get('/v1/backend-functions/price-per-person', function (Request $request, Response $response) use ($app) {
    try {
        $queryParams = $request->getQueryParams();
        $pricingId = isset($queryParams['pricing_id']) ? (int)$queryParams['pricing_id'] : 0;
        ...
        $result = $service->getPricePerPerson($pricingId);
        
        return jsonResponse($response, $result, 'success', 'Price per person retrieved');
...
// Submit ticketing note
$app->post('/v1/backend-functions/ticketing-note', function (Request $request, Response $response) use ($app) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $result = $service->submitTicketingNote($body);
        
        return jsonResponse($response, $result, 'success', 'Ticketing note submitted successfully');
...
// Submit escalation
$app->post('/v1/backend-functions/escalation', function (Request $request, Response $response) use ($app) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $result = $service->submitEscalation($body);
        
        return jsonResponse($response, $result, 'success', 'Escalation submitted successfully');
...
// Process ticketing submission
$app->post('/v1/backend-functions/ticketing-submission', function (Request $request, Response $response) use ($app) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $result = $service->processTicketingSubmission($body);
        
        return jsonResponse($response, $result, 'success', 'Ticketing submission processed successfully');
...
// Update pax field
$app->post('/v1/backend-functions/update-pax-field', function (Request $request, Response $response) use ($app) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $result = $service->updatePaxField($body);
        
        return jsonResponse($response, $result, 'success', 'Pax field updated successfully');
...
// Handle FIT flights
$app->post('/v1/backend-functions/fit-flights', function (Request $request, Response $response) use ($app) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $result = $service->handleFitFlightAction($action, $params);
        
        return jsonResponse($response, $result, 'success', 'FIT flight data retrieved successfully');
...
$app->get('/v1/roster/data', function (Request $request, Response $response) {
    try {
        $queryParams = $request->getQueryParams();
        ...
        $shiftTime = $queryParams['shift_time'] ?? '';
        
        // Debug logging
        error_log("========== ROSTER DATA API DEBUG START ==========");
        error_log("Roster Data API - Month Param: '$monthParam', Year: $year");
        
        // Convert month name to number if needed (e.g., "January" -> 1)
        $month = $monthParam;
        if (!is_numeric($monthParam)) {
            $monthTimestamp = strtotime($monthParam . ' 1, ' . $year);
            if ($monthTimestamp === false) {
                error_log("Roster Data API - Failed to parse month: '$monthParam'");
                throw new Exception('Invalid month format. Use month name (e.g., "January") or number (1-12)', 400);
            }
            $month = (int)date('n', $monthTimestamp);
            error_log("Roster Data API - Converted month name '$monthParam' to number: $month");
        } else {
            $month = (int)$monthParam;
            error_log("Roster Data API - Using numeric month: $month");
        }
        
        error_log("Roster Data API - Calling service with: month=$month, year=$year, team=$team, department=$department, sale_manager=$saleManager, shift_time=$shiftTime");
        
        $service = new \App\Services\RosterService();
        $data = $service->getRosterData($month, $year, $team, $department, $saleManager, $shiftTime);
        
        error_log("Roster Data API - Service returned " . count($data) . " records.");
        error_log("========== ROSTER DATA API DEBUG END ==========");
        
        return jsonResponse($response, $data, 'success', 'Roster data retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        error_log("Roster Data API Error: " . $e->getMessage() . " Trace: " . $e->getTraceAsString());
        return errorResponse($response, $e->getMessage(), $code);
    }
});

// Get roster filter options
$app->get('/v1/roster/filters', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\RosterService();
        $filters = $service->getFilterOptions();
        
        return jsonResponse($response, $filters, 'success', 'Filter options retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Get attendance data for an employee on a specific date
$app->get('/v1/roster/attendance', function (Request $request, Response $response) {
    try {
        $queryParams = $request->getQueryParams();
        $employeeCode = $queryParams['employee_code'] ?? '';
        $date = $queryParams['date'] ?? '';
        
        if (empty($employeeCode) || empty($date)) {
            throw new Exception('employee_code and date are required', 400);
        }
        
        $service = new \App\Services\RosterService();
        $data = $service->getAttendanceData($employeeCode, $date);
        
        return jsonResponse($response, $data, 'success', 'Attendance data retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/roster/attendance/bulk', function (Request $request, Response $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true) ?? [];
        $employeeCodes = $payload['employee_codes'] ?? [];
        $dates = $payload['dates'] ?? [];

        $service = new \App\Services\RosterService();
        $data = $service->getAttendanceDataBulk($employeeCodes, $dates);
...
// G360 Note Report
// ======================================
$app->get('/v1/g360-note-report', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\G360NoteReportService();
        $data = $service->getReport($params);
        return jsonResponse($response, $data, 'success', 'G360 note report retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/g360-note-report/filters', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\G360NoteReportService();
        $data = $service->getFilters();
        return jsonResponse($response, $data, 'success', 'G360 note report filters retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/g360-note-report/counts', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\G360NoteReportService();
        $data = $service->getCounts($params);
        return jsonResponse($response, $data, 'success', 'G360 note report counts retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/payment-recheck/check', function (Request $request, Response $response) {
    try {
        $input = $request->getParsedBody();
        if (!is_array($input)) {
        ...
        $orderId = $input['order_id'] ?? $input['order_id_ajax'] ?? null;
        if ($orderId === null) {
            throw new Exception('order_id is required', 400);
        }
        ...
        $dal = new \App\DAL\PaymentRecheckDAL();
        $service = new \App\Services\PaymentRecheckService($dal);

        $code = $service->checkOrder($orderId);

        return jsonResponse($response, [
            'code' => $code,
        ], 'success', 'Payment recheck evaluated');
    } catch (Exception $e) {
        $status = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $status);
    }
});

$app->post('/v1/payment-status/failure', function (Request $request, Response $response) {
    try {
        $input = $request->getParsedBody();
        if (!is_array($input)) {
            $rawBody = (string)$request->getBody();
        ...
        $dal = new \App\DAL\PaymentFailureDAL();
        $service = new \App\Services\PaymentFailureService($dal);

        $result = $service->recordFailure($input);

        return jsonResponse($response, $result, 'success', 'Payment failure recorded');
    } catch (Exception $e) {
        $status = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $status);
    }
...
// $app->get('/v1/roster/view', function (Request $request, Response $response) {
//     try {
//         $params = $request->getQueryParams();
//         $dal = new \App\DAL\RosterDAL();
//         $service = new \App\Services\RosterService($dal);
//         $data = $service->getRosterData($params);
...
// ============================================
// TRIPS SOLDOUT UPDATER ENDPOINTS
// ============================================

// POST update soldout status
$app->post('/v1/trips/soldout/update', function (Request $request, Response $response) {
    try {
        $body = $request->getParsedBody();
        if (!is_array($body)) {
            $body = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $excludedPostIds = $body['excluded_post_ids'] ?? [60107, 60116];
        
        $service = new \App\Services\SoldoutUpdaterService();
        $result = $service->updateSoldoutStatus($excludedPostIds);
        
        return jsonResponse($response, $result, 'success', 'Soldout status updated successfully');
...
$app->get('/v1/bookings/ypsilon-updated', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $limit = isset($params['limit']) ? (int)$params['limit'] : 100;
        
        $service = new \App\Services\BookingAdminService();
        $result = $service->getYpsilonUpdatedBookings($limit);
        ...
        if (isset($result['bookings']) && is_array($result['bookings'])) {
            foreach ($result['bookings'] as $booking) {
                $bookings[] = [
                    ...
                    'gds_data_updated_on' => $booking['updated_on'] ?? null,
                    'trip_code' => $booking['trip_code'] ?? null
                ];
            }
        }
        
        return jsonResponse($response, $bookings, 'success', 'Ypsilon updated bookings retrieved successfully');
...
// POST capture missing payments for GDeals (WPT)
// This endpoint is specifically for GDeals/WPT payments (merchant_id: 16001455)
$app->post('/v1/payments/asiapay/gdeals/capture', function (Request $request, Response $response) {
    try {
        $body = $request->getParsedBody();
        
        // Get parameters (matching the PHP file's payload structure)
        $queryDate = $body['query_date'] ?? null;
        $merchantId = $body['merchant_id'] ?? '16001455'; // Default to GDeals merchant ID
        
        // Validate query_date format if provided
        if ($queryDate !== null) {
            if (strlen($queryDate) == 8 && preg_match('/^\d{8}$/', $queryDate)) {
                // Already in dmY format (e.g., "15012025")
            } else {
                // Try to convert from other formats
                $queryDate = date('dmY', strtotime($queryDate));
            }
...
// ======================================
// ASIAPAY SETTLEMENT ENDPOINTS
// ======================================

// GET settlement data
$app->get('/v1/asiapay-settlement/data', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $queryDate = $params['query_date'] ?? null;
        $merchantId = $params['merchant_id'] ?? null;
        $source = $params['source'] ?? 'gds';
        
        $service = new \App\Services\AsiaPaySettlementService();
        $data = $service->getSettlementData($queryDate, $merchantId, $source);
        
        if ($data['status'] === 'error') {
            return errorResponse($response, $data['message'] ?? 'Failed to retrieve settlement data', 500);
        }
        
        return jsonResponse($response, $data['data'], 'success', 'Settlement data retrieved successfully');
...
// POST update settlement status
$app->post('/v1/asiapay-settlement/update-status', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        ...
        $transactionType = $params['transaction_type'] ?? 'PAYID';
        $referenceNo = $params['reference_no'] ?? null;
        $source = $params['source'] ?? 'gds';
        
        if (empty($orderId) || empty($amount) || empty($settlementDate)) {
            throw new \Exception('order_id, amount, and settlement_date are required', 400);
        }
        
        $service = new \App\Services\AsiaPaySettlementService();
        $result = $service->updateSettlementStatus($orderId, $amount, $settlementDate, $transactionType, $referenceNo, $source);
        
        if ($result['status'] === 'error') {
            return errorResponse($response, $result['message'] ?? 'Failed to update settlement status', 500);
        }
        
        return jsonResponse($response, $result['data'], 'success', $result['message'] ?? 'Settlement status updated successfully');
...
// POST batch update settlement status
$app->post('/v1/asiapay-settlement/batch-update', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $settlements = $params['settlements'] ?? [];
        
        if (empty($settlements) || !is_array($settlements)) {
            throw new \Exception('settlements array is required', 400);
        }
        
        $service = new \App\Services\AsiaPaySettlementService();
        $result = $service->batchUpdateSettlementStatus($settlements);
        
        if ($result['status'] === 'error') {
            return errorResponse($response, $result['message'] ?? 'Failed to batch update settlement status', 500);
        }
        
        return jsonResponse($response, $result, 'success', $result['message'] ?? 'Batch settlement update completed successfully');
...
// Add contact
$app->post('/v1/whatsapp/contacts', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $data = $service->addContact($phone);

        return jsonResponse($response, $data, 'success', 'Contact processed successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Get contacts list
$app->get('/v1/whatsapp/contacts', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $limit = (int)($params['limit'] ?? 50);

        $service = new \App\Services\WhatsAppService();
        $data = $service->getContactsList($limit);

        return jsonResponse($response, $data, 'success', 'Contacts retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/whatsapp/contacts/check-update', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\WhatsAppService();
        $data = $service->checkContactUpdate();

        return jsonResponse($response, $data, 'success', 'Contact update check completed');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->patch('/v1/whatsapp/conversations/{customer}/in-progress', function (Request $request, Response $response, array $args) {
    try {
        ...
        $user = $payload['user'] ?? '';
        $checked = isset($payload['checked']) ? (bool)$payload['checked'] : false;
        ...
        $service = new \App\Services\WhatsAppService();
        $data = $service->toggleInProgress($customer, $user, $checked);

        return jsonResponse($response, $data, 'success', 'In progress status updated');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/whatsapp/conversations/{customer}/check-update', function (Request $request, Response $response, array $args) {
    try {
        $customer = $args['customer'] ?? '';
        if (empty($customer)) {
            throw new Exception('Customer phone number is required', 400);
        ...
        $service = new \App\Services\WhatsAppService();
        $data = $service->checkConversationUpdate($customer);

        return jsonResponse($response, $data, 'success', 'Conversation update check completed');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/whatsapp/conversations/{customer}/mark-read', function (Request $request, Response $response, array $args) {
    try {
        $customer = $args['customer'] ?? '';
        if (empty($customer)) {
            throw new Exception('Customer phone number is required', 400);
        ...
        $service = new \App\Services\WhatsAppService();
        $data = $service->markMessagesAsRead($customer);

        return jsonResponse($response, $data, 'success', 'Messages marked as read');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/whatsapp/conversations/{customer}/messages', function (Request $request, Response $response, array $args) {
    try {
        $customer = $args['customer'] ?? '';
        if (empty($customer)) {
            throw new Exception('Customer phone number is required', 400);
        ...
        $service = new \App\Services\WhatsAppService();
        $data = $service->getConversationMessages($customer);

        return jsonResponse($response, $data, 'success', 'Conversation messages retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/whatsapp/messages', function (Request $request, Response $response) {
    try {
        ...
        if (empty($message) && empty($request->getUploadedFiles())) {
        ...
        if (!empty($uploadedFiles['file'])) {
            ...
            if ($file->getError() === UPLOAD_ERR_OK) {
                $tempFile = tempnam(sys_get_temp_dir(), 'whatsapp_');
                $file->moveTo($tempFile);
                $filePath = $tempFile;
        ...
        try {
            $data = $service->sendMessage($recipient, $agent, $message, $filePath, $fileName, $mimeType);
...
$app->post('/v1/whatsapp/messages/log', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
        ...
        $service = new \App\Services\WhatsAppMessageLogService();

        $result = $service->log($payload);

        return jsonResponse(
            $response,
            $result,
            'success',
            'WhatsApp message logged successfully',
            201
        );
    } catch (Exception $e) {
...
// ======================================
// ASIAPAY / PAYMENT UTILITIES
// ======================================

// Get asiapay transaction by ref
$app->get('/v1/asiapay/transaction', function (Request $request, Response $response) {
    try {
        $ref = (string)($request->getQueryParams()['ref'] ?? '');
        if ($ref === '') {
            throw new Exception('ref is required', 400);
        }
        $service = new \App\Services\PaymentService();
        $data = $service->getAsiapayTransactionByRef($ref);
        return jsonResponse($response, $data ?? [], 'success', 'Transaction fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/asiapay/transaction', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $service = new \App\Services\PaymentService();
        $ok = $service->createAsiapayTransaction($payload);
        return jsonResponse($response, ['inserted' => $ok], 'success', 'Transaction inserted');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Booking by PNR
$app->get('/v1/bookings/by-pnr/{pnr}', function (Request $request, Response $response, array $args) {
    try {
        $pnr = (string)($args['pnr'] ?? '');
        if ($pnr === '') {
        ...
        $service = new \App\Services\PaymentService();
        $data = $service->getBookingByPnr($pnr);
        return jsonResponse($response, $data ?? [], 'success', 'Booking fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Booking by order_id
$app->get('/v1/bookings/{order_id}', function (Request $request, Response $response, array $args) {
    try {
        ...
        $service = new \App\Services\PaymentService();
        $data = $service->getBooking($orderId);
        return jsonResponse($response, $data ?? [], 'success', 'Booking fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
...
$app->get('/v1/payment-history', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $filters = [
            'reference_no' => $params['reference_no'] ?? null,
            'order_id' => $params['order_id'] ?? null,
            'process_date' => $params['process_date'] ?? null,
            'process_date_alt' => $params['process_date_alt'] ?? null,
            'amount' => $params['amount'] ?? null,
            'payment_method' => $params['payment_method'] ?? null,
        ];
        $service = new \App\Services\PaymentService();
        $data = $service->getPaymentHistory($filters);
        return jsonResponse($response, $data, 'success', 'Payment history fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/payment-history', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $service = new \App\Services\PaymentService();
        $ok = $service->createPaymentHistory($payload);
        return jsonResponse($response, ['inserted' => $ok], 'success', 'Payment history inserted');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/invoices/order/{orderId}', function (Request $request, Response $response, array $args) {
    try {
        $orderId = $args['orderId'] ?? null;
        
        if (empty($orderId)) {
            return errorResponse($response, 'orderId is required', 400);
        }
        
        $service = new \App\Services\InvoiceService();
        $data = $service->getInvoiceByOrderId($orderId);
        
        return jsonResponse($response, $data, 'success', 'Invoice retrieved successfully');
...
// Latest invoice by order_id
$app->get('/v1/payment-invoice/latest', function (Request $request, Response $response) {
    try {
        $orderId = (string)($request->getQueryParams()['order_id'] ?? '');
        if ($orderId === '') {
            throw new Exception('order_id is required', 400);
        }
        $service = new \App\Services\PaymentService();
        $data = $service->getLatestInvoice($orderId);
        return jsonResponse($response, $data ?? [], 'success', 'Latest invoice fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/payment-journal-entry', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $service = new \App\Services\PaymentService();
        $id = $service->createJournalEntry($payload);
        return jsonResponse($response, ['journal_id' => $id], 'success', 'Journal entry created');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Journal line insert
$app->post('/v1/payment-journal-line', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $service = new \App\Services\PaymentService();
        $ok = $service->createJournalLine($payload);
        return jsonResponse($response, ['inserted' => $ok], 'success', 'Journal line created');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// Booking pax by order_id
$app->get('/v1/booking-pax', function (Request $request, Response $response) {
    try {
        $orderId = (string)($request->getQueryParams()['order_id'] ?? '');
        if ($orderId === '') {
            throw new Exception('order_id is required', 400);
        ...
        $service = new \App\Services\PaymentService();
        $data = $service->getBookingPax($orderId);
        return jsonResponse($response, $data, 'success', 'Booking pax fetched');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// ======================================
// AFTER SALES AUDIO ENDPOINTS
// ======================================

$app->get('/v1/after-sales/records', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $limit = isset($params['limit']) ? (int)$params['limit'] : 5000;
        $service = new \App\Services\AfterSalesAudioService();
        $data = $service->listRecords($limit);
        return jsonResponse($response, $data, 'success', 'Records retrieved');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/after-sales/call-ids', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\AfterSalesAudioService();
        $data = $service->listCallIds();
        return jsonResponse($response, $data, 'success', 'Call IDs retrieved');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/after-sales/analysis/{id}', function (Request $request, Response $response, array $args) {
    try {
        $service = new \App\Services\AfterSalesAudioService();
        $data = $service->getAnalysis((int)$args['id']);
        return jsonResponse($response, $data, 'success', 'Analysis retrieved');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/after-sales/analysis', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\AfterSalesAudioService();
        $data = $service->filterAnalysis($request->getQueryParams());
        return jsonResponse($response, $data, 'success', 'Analysis filtered');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->post('/v1/after-sales/remark', function (Request $request, Response $response) {
    try {
        $payload = $request->getParsedBody();
        if (!is_array($payload)) {
            $payload = json_decode($request->getBody()->getContents(), true) ?: [];
        ...
        $text = $payload['text'] ?? '';
        $service = new \App\Services\AfterSalesAudioService();
        $data = $service->saveRemark((int)$id, (string)$metric, (string)$text);
        return jsonResponse($response, $data, 'success', 'Remark saved');
    } catch (Exception $e) {
        $code = getHttpStatusCode($e);
        return errorResponse($response, $e->getMessage(), $code);
    }
...
// ======================================
// TRAMS RECONCILIATION ENDPOINTS
// ======================================

$app->get('/v1/trams-reconciliation', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        ...
        $service = new \App\Services\TramsReconciliationService();
        $data = $service->getReconciliationData($startDate, $endDate, $filter);

        return jsonResponse($response, $data, 'success', 'TRAMS reconciliation data retrieved successfully');
    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
...
$app->get('/v1/customer-view-bookings/search', function (Request $request, Response $response) {
    try {
        ...
        error_log("customer-view-bookings/search - Params: " . print_r($params, true));

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->searchBookings($params);
        
        error_log("customer-view-bookings/search - Success, returning " . (isset($data['total']) ? $data['total'] : 0) . " bookings");

        return jsonResponse($response, $data, 'success', 'Bookings retrieved successfully');

    } catch (Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        error_log("customer-view-bookings/search - Exception: " . $e->getMessage());
        error_log("customer-view-bookings/search - Trace: " . $e->getTraceAsString());
        return errorResponse($response, $e->getMessage(), $code);
    } catch (\Throwable $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        error_log("customer-view-bookings/search - Throwable: " . $e->getMessage());
        error_log("customer-view-bookings/search - Trace: " . $e->getTraceAsString());
        return errorResponse($response, $e->getMessage(), $code);
    }
});
...
// // GET/POST airport info
// $app->map(['GET', 'POST'], '/v1/flight-pricing/airport-info', function (Request $request, Response $response) {
//     try {
//         // Support both GET (query params) and POST (body params)
//         $params = $request->getMethod() === 'POST' 
//             ? ($request->getParsedBody() ?? []) 
...
// POST check flight availability
$app->post('/v1/flights/availability/check', function (Request $request, Response $response) {
    try {
        // Ensure FlightAvailabilityCheckService is loaded
        if (!class_exists('App\Services\FlightAvailabilityCheckService')) {
            $serviceFile = __DIR__ . '/../src/Services/FlightAvailabilityCheckService.php';
            if (file_exists($serviceFile)) {
                require_once $serviceFile;
            } else {
                throw new Exception("FlightAvailabilityCheckService class file not found", 500);
            }
        }
        ...
        $outboundFlightId = $input['outboundFlightId'] ?? null;
        $returnFlightId = $input['returnFlightId'] ?? null;
        
        // Validate required parameters
        if (empty($sessionId)) {
            return errorResponse($response, 'sessionId is required', 400);
        }
        
        if (empty($tarifId)) {
            return errorResponse($response, 'tarifId is required', 400);
        }
        
        // Call service to check availability
        $service = new \App\Services\FlightAvailabilityCheckService();
        $availabilityData = $service->checkAvailability($sessionId, $tarifId, $outboundFlightId, $returnFlightId);
        
        // Optionally save to database if FlightAvailabilityDAL is available
        try {
            if (class_exists('App\DAL\FlightAvailabilityDAL')) {
                $dalFile = __DIR__ . '/../src/DAL/FlightAvailabilityDAL.php';
                if (file_exists($dalFile)) {
                    require_once $dalFile;
                    
                    $dal = new \App\DAL\FlightAvailabilityDAL();
                    
                    // Extract data for database insertion
                    // Get minimum seats from the response
                    $outboundSeat = null;
                    $returnSeat = null;
                    
                    if (isset($availabilityData['minimumAvailableSeats'])) {
                        $seats = $availabilityData['minimumAvailableSeats'];
                        if ($outboundFlightId && isset($seats[$outboundFlightId])) {
                            $outboundSeat = $seats[$outboundFlightId];
                        }
                        if ($returnFlightId && isset($seats[$returnFlightId])) {
                            $returnSeat = $seats[$returnFlightId];
                        }
                    }
                    
                    // Try to extract airport and date information from tarifs/flights
                    $departApt = null;
                    $destApt = null;
                    $departDate = null;
                    $returnDate = null;
                    ...
                    if (isset($availabilityData['tarifs']) && is_array($availabilityData['tarifs'])) {
                        // Try to extract from first tarif/fare/flight
                        foreach ($availabilityData['tarifs'] as $tarif) {
                            if (isset($tarif['fareXRefs']['fareXRef']['flights'])) {
                                $flights = $tarif['fareXRefs']['fareXRef']['flights'];
                                if (is_array($flights) && !empty($flights)) {
                                    $firstFlight = is_array($flights[0]) ? $flights[0] : $flights;
                                    
                                    // Extract from legXRefs if available
                                    if (isset($firstFlight['legXRefs']) && is_array($firstFlight['legXRefs'])) {
                                        $firstLeg = is_array($firstFlight['legXRefs'][0]) ? $firstFlight['legXRefs'][0] : $firstFlight['legXRefs'];
                                        $departApt = $firstLeg['depApt'] ?? null;
                                        $destApt = $firstLeg['dstApt'] ?? null;
                                        $departDate = $firstLeg['depDate'] ?? null;
                                        $airline = $firstLeg['vcr'] ?? null;
                                    }
                                }
                            }
                            break; // Only process first tarif
                        }
                    }
                    
                    // Prepare data for database
                    $dbData = [
                        'user_id' => null, // Can be extracted from session if needed
                        'depart_apt' => $departApt,
                        'dest_apt' => $destApt,
                        'outbound_seat' => $outboundSeat,
                        'return_seat' => $returnSeat,
                    ...
                    // Only insert if we have at least some meaningful data
                    if ($departApt || $destApt || $outboundSeat !== null || $returnSeat !== null) {
                        $availId = $dal->insertAvailabilityCheck($dbData);
                        $availabilityData['_db_record_id'] = $availId;
                    }
                }
            }
        } catch (Exception $dbException) {
            // Log database error but don't fail the request
            error_log("Failed to save availability check to database: " . $dbException->getMessage());
        }
        
        return jsonResponse($response, $availabilityData, 'success', 'Flight availability checked successfully');
        
    } catch (Exception $e) {
        error_log("Exception in flight availability check: " . $e->getMessage());
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
});
...
$app->get('/v1/seat-availability/search', function (Request $request, Response $response) {
    try {
        ...
        // Debug logging - Force output to ensure logs are written
        $debugMsg = "========== SEAT AVAILABILITY API DEBUG START ==========";
        error_log($debugMsg);
        @file_put_contents('php://stderr', $debugMsg . "\n");
        
        error_log("Seat Availability API - Request Params: " . print_r($params, true));
        error_log("Seat Availability API - Filters: " . print_r($filters, true));
        error_log("Seat Availability API - Full URL: " . ($_SERVER['REQUEST_URI'] ?? 'N/A'));
        error_log("Seat Availability API - Request Method: " . ($_SERVER['REQUEST_METHOD'] ?? 'N/A'));
        
        $service = new \App\Services\SeatAvailabilityService();
        error_log("Seat Availability API - Service created, calling getSeatAvailability...");
        
        $data = $service->getSeatAvailability($filters);
        
        error_log("Seat Availability API - Service returned data");
        error_log("Seat Availability API - Data returned with total_count: " . ($data['total_count'] ?? 'N/A'));
        if (isset($data['availability']) && is_array($data['availability'])) {
            error_log("Seat Availability API - Availability array count: " . count($data['availability']));
            if (count($data['availability']) > 0) {
                error_log("Seat Availability API - First availability item: " . print_r($data['availability'][0], true));
            } else {
                error_log("Seat Availability API - Availability array is empty");
            }
        } else {
            error_log("Seat Availability API - No availability key in data or not an array");
        }
        error_log("========== SEAT AVAILABILITY API DEBUG END ==========");
        
        return jsonResponse($response, $data, 'success', 'Seat availability retrieved successfully');
        
    } catch (Exception $e) {
        error_log("Seat Availability API - Exception: " . $e->getMessage());
        error_log("========== SEAT AVAILABILITY API DEBUG END (ERROR) ==========");
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        return errorResponse($response, $e->getMessage(), $code);
    }
});
...
// GET seat availability export (CSV format)
// Matches original export logic from tpl_seat_availability_internal.php
$app->get('/v1/seat-availability/export', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        
        $filters = [
            'airline_code' => $params['trip'] ?? $params['airline_code'] ?? null,
            'route' => $params['route'] ?? null,
            'date_from' => $params['date'] ?? $params['date_from'] ?? null,
            'date_to' => $params['date'] ?? $params['date_to'] ?? null
        ];
        
        $service = new \App\Services\SeatAvailabilityService();
        $data = $service->getSeatAvailabilityForExport($filters);
        
        return jsonResponse($response, $data, 'success', 'Seat availability export data retrieved successfully');
...
$app->map(['GET', 'POST'], '/v1/employee-performance/yearly/{tsr}', function (Request $request, Response $response, array $args) {
    try {
        $tsr = $args['tsr'] ?? null;
...
$app->get('/v1/auto-cancellation/midnight-fullpayment', function (Request $request, Response $response) {
    try {
        error_log("========== MIDNIGHT API DEBUG START ==========");
        
        // Check if diagnostic mode is requested
        $queryParams = $request->getQueryParams();
        $includeDiagnostics = isset($queryParams['diagnostics']) && $queryParams['diagnostics'] == '1';
        
        // Test: Get raw data from DAL first
        $dal = new \App\DAL\AutoCancellationMidnightDAL();
        $rawBookings = $dal->getBookingsForFullPaymentCancellation();
        error_log("Midnight API: DAL returned " . count($rawBookings) . " raw bookings");
        
        if (count($rawBookings) > 0) {
            error_log("Midnight API: First raw booking: " . print_r($rawBookings[0], true));
        }
        
        $service = new \App\Services\AutoCancellationMidnightService();
        $data = $service->getBookingsForFullPaymentCancellation();
        
        error_log("Midnight API: Service returned " . count($data['bookings'] ?? []) . " bookings");
        
        // If no bookings found and diagnostics requested, include diagnostic info
        if (count($data['bookings'] ?? []) == 0 && $includeDiagnostics) {
            $diagnostics = $dal->getDiagnosticInfo();
            $data['diagnostics'] = $diagnostics;
        }
        
        error_log("========== MIDNIGHT API DEBUG END ==========");
        
        return jsonResponse($response, $data, 'success', 'Bookings for full payment cancellation retrieved successfully');
        
    } catch (\Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        error_log("Midnight API Error: " . $e->getMessage() . " Trace: " . $e->getTraceAsString());
        return errorResponse($response, $e->getMessage(), $code);
    }
});
...
// ======================================
// AZUPAY PAYMENTS ENDPOINTS
// ======================================

// GET custom payments with filters
$app->get('/v1/azupay-payments/custom-payments', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        
        $service = new \App\Services\AzupayPaymentsService();
        $data = $service->getCustomPayments($params);
        
        return jsonResponse($response, $data, 'success', 'Custom payments retrieved successfully');
...
// GET pending payments for cron processing
$app->get('/v1/azupay-cron/pending-payments', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        
        $service = new \App\Services\AzupayPaymentsService();
        $data = $service->getPendingPaymentsForCron($params);
        
        return jsonResponse($response, $data, 'success', 'Pending payments retrieved successfully');
...
// POST Preview Stock Adjustment
$app->post('/v1/stock/adjustment/preview', function (Request $request, Response $response) {
    try {
        $input = $request->getParsedBody();
        if (!is_array($input)) {
            $rawBody = (string)$request->getBody();
        ...
        return jsonResponse($response, [
            'records' => $records,
            'count' => count($records)
        ], 'success', 'Stock adjustment preview retrieved successfully');
...
// POST Apply Stock Adjustment
$app->post('/v1/stock/adjustment/apply', function (Request $request, Response $response) {
    try {
        $input = $request->getParsedBody();
        if (!is_array($input)) {
            $rawBody = (string)$request->getBody();
        ...
        foreach ($records as $record) {
            ...
            foreach ($required as $field) {
                if (!isset($record[$field])) {
                    throw new Exception("Field '{$field}' is required in record", 400);
                }
            }
...
// STOCK PRODUCT MANAGER - ADD PRODUCT TO STOCK MANAGER
$app->post('/v1/stock-product-manager/products', function (Request $request, Response $response) {
	try {
		$params = $request->getParsedBody();
		if (empty($params)) {
			throw new Exception('Product data is required', 400);
		}
		$service = new \App\Services\StockProductManagerService();
		$result = $service->addProductToStockManager($params);
		return jsonResponse($response, $result, 'success', 'Product added to stock manager successfully', 201);
	}
	catch (Exception $e) {
		$code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
		return errorResponse($response, $e->getMessage(), $code);
...
$app->map(['GET', 'DELETE', 'POST'], '/v1/test-route', function (Request $request, Response $response) {
    error_log("[" . date('Y-m-d H:i:s') . "] Test route hit!\n", 3, __DIR__ . '/../error.log');
    return jsonResponse($response, ['test' => 'success'], 'success', 'Test route works');
});

// DELETE/POST wipe realtime bookings for a specific date
$app->map(['DELETE', 'POST'], '/v1/realtime-bookings/wipe/yesterday', function (Request $request, Response $response) {
    error_log("[" . date('Y-m-d H:i:s') . "] Realtime booking wipe route hit!\n", 3, __DIR__ . '/../error.log');
    try {
        $body = $request->getParsedBody() ?? [];
        $targetDate = $body['date'] ?? null;
        
        $service = new \App\Services\RealtimeBookingService();
        $result = $service->wipeRealtimeBookings($targetDate);
        
        return jsonResponse($response, $result, 'success', $result['message'] ?? 'Realtime bookings wiped successfully');
        
    } catch (\Exception $e) {
        $code = ($e->getCode() >= 400 && $e->getCode() < 600) ? $e->getCode() : 500;
        error_log("Realtime Booking Wipe Error: " . $e->getMessage());
        return errorResponse($response, $e->getMessage(), $code);
    }
});

$app->get('/v1/manage-wp-order-v3/payment-history', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaymentHistory($orderId);

        return jsonResponse($response, $data, 'success', 'Payment history retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/booking-summary', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingSummary($orderId);

        return jsonResponse($response, $data, 'success', 'Booking summary retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/bpay-awaiting-event', function (Request $request, Response $response) {
    try {
        $orderId = (int)($request->getQueryParams()['order_id'] ?? 0);
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->hasBpayAwaitingEvent($orderId);

        return jsonResponse($response, $data, 'success', 'BPAY awaiting event checked');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});
...
$app->get('/v1/manage-wp-order-v3/bpay-approved-event', function (Request $request, Response $response) {
    try {
        $orderId = (int)($request->getQueryParams()['order_id'] ?? 0);
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->hasBpayApprovedEvent($orderId);

        return jsonResponse($response, $data, 'success', 'BPAY approved event checked');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});
...
$app->post('/v1/manage-wp-order-v3/payment-status/auto-finalize', function (Request $request, Response $response) {
    try {
        $body = json_decode((string)$request->getBody(), true);

        if (empty($body['order_id']) || empty($body['new_payment_status'])) {
            throw new \InvalidArgumentException('Missing required fields');
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->autoFinalizePaymentStatus(
            (int)$body['order_id'],
            $body['new_payment_status'],
            $body['order_type'] ?? ''
        );

        return jsonResponse($response, ['updated' => true], 'success', 'Payment status finalized');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/payment-base-data', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaymentBaseDataForStatus($orderId);

        return jsonResponse($response, $data, 'success', 'Payment base data retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->post('/v1/manage-wp-order-v3/fix-return-bookings', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->fixReturnBookings();
...
$app->get('/v1/manage-wp-order-v3/bookings-list', function ($req, $res) {
    $service = new \App\Services\ManageWpOrderV3Service();
    return jsonResponse($res, $service->getBookingsList());
});

$app->get('/v1/manage-wp-order-v3/return-count', function ($req, $res) {
    $q = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, $service->getReturnCount(
        (int)$q['order_id'],
        $q['co_order_id'] ?? ''
    ));
});

$app->get('/v1/manage-wp-order-v3/movement-count', function ($req, $res) {
    $q = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, $service->getMovementCount(
        (int)$q['order_id'],
        (int)$q['product_id'],
        $q['co_order_id'] ?? ''
    ));
...
$app->get('/v1/manage-wp-order-v3/divide-bookings', function (Request $request, Response $response) {
    try {
        $q = $request->getQueryParams();

        $orderId    = (int)($q['order_id'] ?? 0);
        $coOrderId  = $q['co_order_id'] ?? '';
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getDivideBookings(
            $orderId,
            $coOrderId,
            $productId
        );

        return jsonResponse($response, $data, 'success', 'Divide bookings retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/divide-pax', function (Request $request, Response $response) {
    try {
        $q = $request->getQueryParams();
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getDividePaxList(
            $orderId,
            $coOrderId,
            $productId
        );

        return jsonResponse($response, $data, 'success', 'Divide pax retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->post('/v1/manage-wp-order-v3/divide-order', function (Request $req, Response $res) {
    try {
        ...
        return jsonResponse($res, ['ok' => true], 'success', 'Order divided');
    } catch (\Throwable $e) {
        return errorResponse($res, $e->getMessage(), 400);
    }
});

$app->post('/v1/manage-wp-order-v3/save-movement', function (Request $req, Response $res) {
    try {
        $body = json_decode((string)$req->getBody(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->saveMovement($body);

        return jsonResponse($res, ['ok' => true], 'success', 'Movement saved');
    } catch (\Throwable $e) {
        return errorResponse($res, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/movement-booking', function ($req, $res) {
    try {
        $q = $req->getQueryParams();

        $orderId   = $q['order_id'] ?? $q['orderid'] ?? '';
        $productId = $q['product_id'] ?? $q['productid'] ?? '';
        $coOrderId = $q['co_order_id'] ?? $q['co_orderid'] ?? '';

        if (empty($orderId) || empty($productId)) {
            throw new Exception('Missing params: order_id and product_id are required', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getMovementBooking(
            $orderId,
            $productId,
            $coOrderId
        );

        return jsonResponse($res, $data, 'success', 'OK');
    } catch (Throwable $e) {
        return errorResponse($res, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/update-ticket/booking-pax', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        $orderId = $q['order_id'] ?? '';
        $paxId   = $q['pax_id'] ?? '';
        ...
        if ($orderId === '' || $paxId === '') {
            throw new Exception('Missing params: order_id, pax_id', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getUpdateTicketBookingPax($orderId, $paxId);
...
$app->get('/v1/manage-wp-order-v3/update-ticket/airline-code', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        $iata = $q['iata_code'] ?? '';

        if ($iata === '') {
            throw new Exception('Missing params: iata_code', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getAirlineCodeByIata($iata);
...
$app->post('/v1/manage-wp-order-v3/update-ticket/save', function ($req, $res) {
    try {
        $data = json_decode((string)$req->getBody(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->saveTicketNumber($data);

        return jsonResponse($res, ['ok' => true], 'success', 'Ticket saved');
    } catch (\Throwable $e) {
        return errorResponse($res, $e->getMessage(), 400);
    }
});

$app->get('/v1/manage-wp-order-v3/view-booking', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        ...
        if ($orderId === '') {
            throw new Exception('Missing order_id', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getViewBooking($orderId, $coOrderId);
...
$app->get('/v1/manage-wp-order-v3/booking-by-order-id', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        $orderId = $q['order_id'] ?? '';
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingByOrderId($orderId);
...
$app->get('/v1/manage-wp-order-v3/view-booking-v2', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getViewBookingV2($orderId, $coOrderId);
...
$app->get('/v1/manage-wp-order-v3/postmeta', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        ...
        if ($postId === '' || $metaKey === '') {
            throw new Exception('Missing post_id or meta_key', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPostMetaValue($postId, $metaKey);
...
$app->get('/v1/manage-wp-order-v3/history-of-updates', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        ...
        if ($typeId === '' || $metaKey === '') {
            throw new Exception('Missing type_id or meta_key', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getHistoryOfUpdates($typeId, $metaKey);
...
$app->get('/v1/manage-wp-order-v3/booking-pax-by-order', function ($req, $res) {
    try {
        $orderId = $req->getQueryParams()['order_id'] ?? '';
        if ($orderId === '') {
            throw new Exception('Missing order_id', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingPaxByOrderId($orderId);
...
$app->get('/v1/manage-wp-order-v3/payment-history-for-balance', function ($req, $res) {
    try {
        $orderId = $req->getQueryParams()['order_id'] ?? '';
        if ($orderId === '') {
            throw new Exception('Missing order_id', 400);
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaymentHistoryForBalance($orderId);
...
// DC payment history
$app->get('/v1/manage-wp-order-v3/dc-payment-history', function ($request, $response) {
    $orderId = $request->getQueryParams()['order_id'] ?? '';
    if ($orderId === '') {
        return errorResponse($response, 'order_id required', 400);
    }

    $service = new \App\Services\ManageWpOrderV3Service();
    $rows = $service->getDcPaymentHistory($orderId);

    return jsonResponse($response, $rows);
});

// DC custom requested payments
$app->get('/v1/manage-wp-order-v3/dc-custom-payments', function ($request, $response) {
    $reference = $request->getQueryParams()['reference'] ?? '';
    if ($reference === '') {
        return errorResponse($response, 'reference required', 400);
    }

    $service = new \App\Services\ManageWpOrderV3Service();
    $rows = $service->getDcCustomPayments($reference);

    return jsonResponse($response, $rows);
});

$app->get('/v1/manage-wp-order-v3/todo-tasks', function ($request, $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? ''; 

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTodoTaskListByOrder($orderId);
...
$app->get('/v1/manage-wp-order-v3/booking-edit', function ($request, $response) {
    try {
        $params = $request->getQueryParams();

        $orderId   = $params['order_id'] ?? '';
        $productId = $params['product_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingForEdit($orderId, $productId, $coOrderId);
...
$app->get('/v1/manage-wp-order-v3/booking-pax', function ($request, $response) {
    try {
        $params = $request->getQueryParams();
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingPaxForEdit($orderId, $productId, $coOrderId);
...
$app->post('/v1/manage-wp-order-v3/update-payment-status', function ($request, $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->updateBookingPaymentStatus2($payload);
...
$app->get('/v1/manage-wp-order-v3/pax-duplicate-count', function ($request, $response) {
    try {
        $params = $request->getQueryParams();
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $count = $service->countPaxByOrderIdExceptProduct($orderId, $productId);

        return jsonResponse($response, ['count' => $count]);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
});

$app->get('/v1/manage-wp-order-v3/booking-pax-by-auto-id', function ($request, $response) {
    try {
        $params = $request->getQueryParams();
        $autoId = $params['auto_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $row = $service->getBookingPaxByAutoId($autoId);
...
$app->post('/v1/manage-wp-order-v3/pax/update-single-field', function ($request, $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->updateBookingPaxSingleField(
            $payload['order_id']   ?? '',
            $payload['product_id'] ?? '',
            $payload['auto_id']    ?? '',
            $payload['column']     ?? '',
...
$app->post('/v1/manage-wp-order-v3/pax/update-single-field-realtime', function ($request, $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->updateBookingPaxRealtimeSingleField(
            $payload['order_id']   ?? '',
            $payload['product_id'] ?? '',
            $payload['fname']      ?? '',
            $payload['lname']      ?? '',
...
$app->post('/v1/manage-wp-order-v3/pax/insert-update-history', function ($request, $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->insertBookingPaxUpdateHistory(
            $payload['order_id']    ?? '',
            $payload['pax_auto_id'] ?? '',
            $payload['column']      ?? '',
            $payload['value']       ?? '',
...
$app->post('/v1/manage-wp-order-v3/pax/update-late-modified', function ($request, $response) {
    try {
        $payload = json_decode($request->getBody()->getContents(), true);

        $service = new \App\Services\ManageWpOrderV3Service();
        $service->updateBookingPaxLateModified(
            $payload['auto_id']       ?? '',
...
// 2026-01-13
/**
 * GET /v1/manage-wp-order-v3/bookings
 * Legacy-equivalent booking full row query
 */
$app->get('/v1/manage-wp-order-v3/bookings', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getBookingsByOrderProductCoOrder(
            $orderId,
            $productId,
            $coOrderId
        );
...
/**
 * GET /v1/manage-wp-order-v3/booking-pax
 * Legacy-equivalent booking pax query
 */
$app->get('/v1/manage-wp-order-v3/booking-pax2', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getBookingPaxByOrderProductCoOrder(
            $orderId,
            $productId,
            $coOrderId
        );
...
/**
 * POST /v1/manage-wp-order-v3/update-payment-status
 * Write-only endpoint for legacy payment status update
 */
$app->post('/v1/manage-wp-order-v3/update-payment-status3', function ($request, $response) {
    // Decode JSON payload from legacy tpl
    $input = json_decode($request->getBody()->getContents(), true);

    // Execute legacy-equivalent service logic
    $service = new \App\Services\ManageWpOrderV3Service();
    $result = $service->updatePaymentStatus($input);
...
/**
 * GET /v1/manage-wp-order-v3/pax-duplicate-check
 * Legacy pax duplicate and checker API
 */
$app->get('/v1/manage-wp-order-v3/pax-duplicate-check', function ($request, $response) {
    // Read legacy query parameters
    $params = $request->getQueryParams();

    $service = new \App\Services\ManageWpOrderV3Service();
    $result  = $service->getPaxDuplicateAndList($params);

    return jsonResponse($response, $result);
});

/**
 * POST /v1/manage-wp-order-v3/update-pax-field
 * Legacy pax loop-edit write API
 */
$app->post('/v1/manage-wp-order-v3/update-pax-field', function ($request, $response) {
    $input = json_decode($request->getBody()->getContents(), true);

    $service = new \App\Services\ManageWpOrderV3Service();
    $service->updatePaxFieldValue($input);

    return jsonResponse($response, ['success' => true]);
});

/**
 * POST /v1/manage-wp-order-v3/update-pax-late-modified
 * Legacy pax audit update API
 */
$app->post('/v1/manage-wp-order-v3/update-pax-late-modified', function ($request, $response) {
    $input = json_decode($request->getBody()->getContents(), true);

    $service = new \App\Services\ManageWpOrderV3Service();
    $service->updatePaxLateModified($input);

    return jsonResponse($response, ['success' => true]);
});

/**
 * POST /v1/manage-wp-order-v3/amadeus-payment-status-log
 * Legacy Amadeus payment status logging API
 */
$app->post('/v1/manage-wp-order-v3/amadeus-payment-status-log', function ($request, $response) {
    $input = json_decode($request->getBody()->getContents(), true);

    $service = new \App\Services\ManageWpOrderV3Service();
    $service->logAmadeusPaymentStatusUpdate($input);

    return jsonResponse($response, ['success' => true]);
});


$app->get('/v1/manage-wp-order-v3/booking-by-order-id2', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getBookingByOrderId2($params)
    ]);
});

$app->get('/v1/manage-wp-order-v3/payment-history-by-order-id', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getPaymentHistoryByOrderId($params)
    ]);
});

$app->get('/v1/manage-wp-order-v3/task-history-by-order-id', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getTaskHistoryByOrderId($params)
    ]);
});

$app->get('/v1/manage-wp-order-v3/movement-history-by-order-id', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getMovementHistoryByOrderId($params)
    ]);
});

$app->get('/v1/manage-wp-order-v3/booking-update-history-by-order-id', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getBookingUpdateHistoryByOrderId($params)
    ]);
});

$app->get('/v1/manage-wp-order-v3/pax-by-order-id-auto-id', function ($req, $res) {
    $params = $req->getQueryParams();
    $service = new \App\Services\ManageWpOrderV3Service();

    return jsonResponse($res, [
        'data' => $service->getPaxByOrderIdAndAutoId($params)
    ]);
});
...
// 2026-01-14
/**
 * GET /v1/manage-wp-order-v3/trips-by-order-id
 */
$app->get('/v1/manage-wp-order-v3/trips-by-order-id', function ($req, $res) {
    $orderId = $req->getQueryParams()['order_id'] ?? '';
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $data = $service->getTripsByOrderId($orderId);
 ...
 * GET /v1/manage-wp-order-v3/booking-pax
 */
$app->get('/v1/manage-wp-order-v3/booking-pax3', function (
    Request $request,
    Response $response
) {
    try {
...
/**
 * GET /v1/manage-wp-order-v3/booking-by-order-product-co-order
 * ManagePNR - booking base info
 */
$app->get('/v1/manage-wp-order-v3/booking-by-order-product-co-order', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingForManagePNR(
            $orderId,
            $productId,
            $coOrderId
        );
...
/**
 * save ticket info
 * POST /v1/manage-wp-order-v3/managepnr/update-pax
 */
$app->post('/v1/manage-wp-order-v3/managepnr/update-pax', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $service->updateManagePNRPax($input);
...
/**
 * GET /v1/manage-wp-order-v3/booking-by-order-product
 */
$app->get('/v1/manage-wp-order-v3/booking-by-order-product', function (
    Request $request,
    Response $response
) {
    try {
        ...
        if ($orderId === '' || $productId === '') {
            throw new Exception('order_id and product_id are required', 400);
        }

        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingByOrderProduct($orderId, $productId);
...
$app->get('/v1/manage-wp-order-v3/todo-tasks2', function (
    Request $request,
    Response $response
) {
    try {
...
$app->get('/v1/users/list', function (
    Request $request,
    Response $response
) {
    try {
...
//save comment
$app->post('/v1/manage-wp-order-v3/todo-task/status', function (
    Request $request,
    Response $response
) {
    $input = json_decode($request->getBody()->getContents(), true);
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $service->updateTodoTaskStatus($input);

    return jsonResponse($response, true, 'success');
});

$app->post('/v1/manage-wp-order-v3/todo-task', function (
    Request $request,
    Response $response
) {
    $input = json_decode($request->getBody()->getContents(), true);
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $service->createTodoTask($input);

    return jsonResponse($response, true, 'success');
});

// 2026-01-15
$app->get('/v1/todo-tasks2', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->searchTasks($params);
...
$app->get('/v1/todo-task-chat', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getChatByRequestId($params['request_id'] ?? '');
...
/**
 * GET /v1/manage-wp-order-v3/datechange-requests
 * Legacy equivalent of datechange_request query
 */
$app->get('/v1/manage-wp-order-v3/datechange-requests', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getDateChangeRequestsByOrderId($orderId);
...
/**
 * GET /v1/manage-wp-order-v3/todo-tasks3
 * Exclude date change tasks
 */
$app->get('/v1/manage-wp-order-v3/todo-tasks3', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getTodoTasksByOrderIdExcludeDateChange($orderId);
...
/**
 * GET /v1/manage-wp-order-v3/todo-task-chat
 */
$app->get('/v1/manage-wp-order-v3/todo-task-chat2', function (
    Request $request,
    Response $response
) {
    try {
        $requestId = $request->getQueryParams()['request_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getTodoTaskChatByRequestId($requestId);
...
/**
 * GET /v1/manage-wp-order-v3/general-todo-tasks
 */
$app->get('/v1/manage-wp-order-v3/general-todo-tasks', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getGeneralTodoTasksByOrderId($orderId);
...
/**
 * GET /v1/manage-wp-order-v3/refund-todo-tasks
 */
$app->get('/v1/manage-wp-order-v3/refund-todo-tasks', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getRefundTodoTasksByOrderId($orderId);
...
/**
 * GET /v1/manage-wp-order-v3/stock-product
 */
$app->get('/v1/manage-wp-order-v3/stock-product', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $row = $service->getStockProductByTripAndDate($tripCode, $travelDate);
...
$app->get('/v1/manage-wp-order-v3/booking-pax4', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getBookingPaxByOrderId4($orderId);
...
$app->get('/v1/manage-wp-order-v3/user-portal-requests', function (
    Request $request,
    Response $response
) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $pnrs = isset($params['pnrs'])
            ? explode(',', $params['pnrs'])
            : [];

        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getUserPortalRequestsByRefs($orderId, $pnrs);
...
$app->get('/v1/manage-wp-order-v3/datechange-by-portal-case', function (
    Request $request,
    Response $response
) {
    try {
        $caseId = $request->getQueryParams()['portal_case_id'] ?? '';

        $service = new \App\Services\ManageWpOrderV3Service();
        $row = $service->getDateChangeByPortalCaseId($caseId);
...
$app->post('/v1/manage-wp-order-v3/azupay-dc-payment-request', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $currentUserLogin = $payload['requested_by'] ?? 'system';

        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->createAzupayDcPaymentRequest($payload, $currentUserLogin);

        if (!($result['ok'] ?? false)) {
            return errorResponse($response, $result['message'] ?? 'Failed', 400);
        }

        return jsonResponse($response, $result['data'] ?? [], 'success', $result['message'] ?? null);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
});
...
/**
 * Admin: Get AzuPay Refund Requests for Review
 */
$app->get('/v1/user-portal/admin/refunds/review', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\UserPortalService();
        $data = $service->getAzupayRefundRequests($params);
        return jsonResponse($response, $data);
    } catch (Exception $e) {
        return errorResponse($response, $e->getMessage(), $e->getCode() ?: 500);
    }
});

/**
 * Admin: Approve AzuPay Refund Request
 */
$app->post('/v1/user-portal/admin/refunds/approve/{autoId}', function (Request $request, Response $response, array $args) {
    try {
        $autoId = $args['autoId'];
        $params = $request->getQueryParams();
        ...
        $service = new \App\Services\UserPortalService();
        $result = $service->approveAzupayRefund($autoId, $user);
...
/**
 * Admin: Get Refund Manager Data for a Case
 */
$app->get('/v1/user-portal/admin/refunds/{caseId}', function (Request $request, Response $response, array $args) {
    try {
        $caseId = $args['caseId'];
        $service = new \App\Services\UserPortalService();
        $data = $service->getRefundManagerData($caseId);
        return jsonResponse($response, $data);
    } catch (Exception $e) {
        return errorResponse($response, $e->getMessage(), $e->getCode() ?: 500);
    }
...
$app->post('/v1/manage-wp-order-v3/partial-datechange-accept', function (
    Request $request,
    Response $response
) {
    $params = $request->getParsedBody();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $result = $service->partialDatechangeAccept($params);

    return jsonResponse($response, $result);
});

$app->post('/v1/manage-wp-order-v3/datechange-accept', function (
    Request $request,
    Response $response
) {
    $params = $request->getParsedBody();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $result = $service->datechangeAccept($params);

    return jsonResponse($response, $result);
});

$app->post('/v1/manage-wp-order-v3/datechange-decline', function (
    Request $request,
    Response $response
) {
    $params = $request->getParsedBody();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $result = $service->datechangeDecline($params);

    return jsonResponse($response, $result);
});

$app->post('/v1/manage-wp-order-v3/datechange-payment-status', function (
    Request $request,
    Response $response
) {
    $params = $request->getParsedBody();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $result = $service->updateDatechangePaymentStatus($params);

    return jsonResponse($response, $result);
});

$app->get('/v1/manage-wp-order-v3/datechange-form-data', function (
    Request $request,
    Response $response
) {
    $params = $request->getQueryParams();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $data = $service->getDatechangeFormData($orderId, $coOrderId);

    return jsonResponse($response, $data);
});

$app->get('/v1/manage-wp-order-v3/getTodoTasksByOrderAndType', function (
    Request $request,
    Response $response
) {
    $params = $request->getQueryParams();
    ...
    $orderId     = $params['order_id'] ?? '';
    $commentType = $params['comment_type'] ?? '';

    $service = new \App\Services\ManageWpOrderV3Service();
    $rows = $service->getTodoTasksByOrderAndType($orderId, $commentType);

    return jsonResponse($response, $rows);
});

$app->get('/v1/manage-wp-order-v3/payment-adjustments', function (
    Request $request,
    Response $response
) {
    $params = $request->getQueryParams();
    ...
    $service = new \App\Services\ManageWpOrderV3Service();
    $rows = $service->getPaymentAdjustments($orderId, $status);

    return jsonResponse($response, $rows);
});

//update user payment transfer
$app->post('/v1/manage-wp-order-v3/payment-adjustment', function ($request, $response) {
    try {
        $body = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();

        if (($body['update_user_payment_transfer'] ?? '') === 'Deny') {
            $data = $service->denyPaymentTransfer(
                $body['auto_id'],
                $body['new_order_id']
            );
            return jsonResponse($response, $data);
        ...
        if (($body['update_user_payment_transfer'] ?? '') === 'Approve') {
            $data = $service->approvePaymentTransfer($body);
            return jsonResponse($response, $data);
        }

        return jsonResponse($response, ['error' => 'invalid_action'], 400);
...
$app->get('/v1/todo-tasks/miscellaneous-by-order-id', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getMiscellaneousTasksByOrderId($orderId);
...
$app->post('/v1/todo-tasks/update-status', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service->updateTodoTaskStatus2(
            $params['order_id'] ?? '',
            $params['auto_id'] ?? '',
            $params['status'] ?? '',
            $params['updated_by'] ?? '',
            $params['status_updated_on'] ?? ''
        );
...
$app->post('/v1/todo-task-chat/insert', function (
    Request $request,
    Response $response
) {
    try {
...
$app->post('/v1/todo-tasks/insert', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service->insertTodoTask(
            $params['order_id']          ?? '',
            $params['product_id']        ?? '',
            $params['comment_by']        ?? '',
            $params['comment_date_time'] ?? '',
            $params['comment_type']      ?? '',
            $params['comment']           ?? '',
            $params['assigned_to']       ?? '',
...
$app->get('/v1/manage-wp-order-v3/ticketing-deadline', function (
    Request $request,
    Response $response
) {
    try {
        $service = new \App\Services\ManageWpOrderV3Service();
        $rows = $service->getTicketingDeadlineList();
...
$app->get('/v1/manage-wp-order-v3/bpay-receipts', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $rows = $service->getBpayReceiptList(
            $params['id'] ?? '',
            $params['date'] ?? '',
            $params['status'] ?? ''
        );
...
$app->post('/v1/manage-wp-order-v3/delete-schedule-change', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service->clearBookingNoteForAgents(
            $params['order_id'] ?? ''
        );
...
// 2026-01-16
$app->post('/v1/manage-wp-order-v3/bpay-receipts/update-status', function (
    Request $request,
    Response $response
) {
    try {
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->updateBpayReceiptStatus($params);
...
$app->get('/v1/manage-wp-order-v3/search/gds-check', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getGdsUpdateCheck($params);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/search', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->searchBookings($params);
...
$app->get('/v1/manage-wp-order-v3/pax/any-contact', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $column = $params['column'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getAnyPaxContact($orderId, $column);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/meta-value', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $metaKey = $params['meta_key'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getHistoryMetaValueOnly($orderId, $metaKey);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/meta-values', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $metaKeys = $params['meta_keys'] ?? [];
        if (is_string($metaKeys)) {
            $metaKeys = explode(',', $metaKeys);
        }
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getHistoryMetaValues($orderId, $metaKeys);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-details-consolidated', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingDetailsConsolidated($orderId, $orderType, $departureCode, $destinationCode);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/changelog/meta-values', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $metaKeys = $params['meta_keys'] ?? [];
        ...
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getHistoryMetaValuesFromChangelog($orderId, $metaKeys);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/by-meta-key', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $metaKey = $params['meta_key'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getHistoryOfUpdatesByMetaKey($orderId, $metaKey);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/latest-meta-change', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getLatestMetaChange($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/changelog-dividers', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getChangelogDividers($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/payment/followup-agent', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaymentFollowupAgent($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/predeparture-check', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPredepartureCheck($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/airport/check-country', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $code = $params['code'] ?? '';
        $countries = isset($params['countries']) ? explode(',', $params['countries']) : [];
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->checkAirportCountry($code, $countries);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/booking/status-checklist', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingStatusChecklist($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/booking/dates-diff', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingDatesDiff($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/summary/trips', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryTrips($orderId, $coOrderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/summary/pax-pnr', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $productId = $params['product_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryPaxPnr($orderId, $productId, $coOrderId);
...
$app->get('/v1/manage-wp-order-v3/summary/oid', function (Request $request, Response $response) {
    try {
        $pnr = $request->getQueryParams()['pnr'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryOid($pnr);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/summary/trips-agent', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryTripsAgent($orderId, $coOrderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/summary/oid-generic', function (Request $request, Response $response) {
    try {
        $pnr = $request->getQueryParams()['pnr'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryOidGeneric($pnr);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/summary/trips-gds', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSummaryTripsGds($orderId, $coOrderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/pax-details-by-pnr', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $pnr = $params['pnr'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaxDetailsByPnr($orderId, $pnr);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/bookings-by-order-coorder', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingsByOrderAndCoOrderList($orderId, $coOrderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/bookings-by-order-coorder-ordered', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingsByOrderAndCoOrderListOrdered($orderId, $coOrderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/post-field', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $postId = $params['post_id'] ?? '';
        $fieldName = $params['field_name'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPostField($postId, $fieldName);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/option', function (Request $request, Response $response) {
    try {
        $optionName = $request->getQueryParams()['option_name'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getOption($optionName);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/ticket-issue-loss', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $productId = $params['product_id'] ?? '';
        $travelDate = $params['travel_date'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTicketIssueLoss($productId, $travelDate);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/order-items-data', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getOrderItemsData($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/post-meta', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $metaKey = $params['meta_key'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPostMeta($orderId, $metaKey);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/bank-account2', function (Request $request, Response $response) {
    try {
        $bankId = $request->getQueryParams()['bank_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBankAccount($bankId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/payment-history-filtered', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $payTypes = isset($params['pay_types']) ? explode(',', $params['pay_types']) : [];
        $includeEmpty = ($params['include_empty'] ?? '0') === '1';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaymentHistoryWithFilters($orderId, $payTypes, $includeEmpty);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/ticket-history-with-pax', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTicketHistoryWithPax($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/v1/manage-wp-order-v3/payment-transfer-execute', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->executePaymentTransfer($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/payment-conversation/latest', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $msgType = $params['msg_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getLatestPaymentConversation($orderId, $msgType);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-update-history', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingUpdateHistory($params);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-update-history/all', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBookingUpdateHistoryAllByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/pax-by-auto-id', function (Request $request, Response $response) {
    try {
        $autoId = $request->getQueryParams()['auto_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPaxByAutoId($autoId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/g360-history', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getG360HistoryByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/manage-wp-order-v3/payment-transfer/user-execute', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->executeUserPaymentTransfer($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/escalations', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getEscalationsWithResponseStatus($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/escalation/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitEscalation($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/email-history/latest', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $emailType = $params['email_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getLatestEmailHistory($orderId, $emailType);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/email-history/record', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->recordEmailHistory($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/booking-history/record-detailed', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->recordBookingHistoryDetailed($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-pax/first', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingPaxFirst($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/bookings-by-type', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $orderType = $params['order_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingsByOrderIdAndType($orderId, $orderType);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-pax-by-product', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $productId = $params['product_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingPaxByOrderAndProduct($orderId, $productId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-pax-by-product-type', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $productId = $params['product_id'] ?? '';
        $orderType = $params['order_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingPaxByOrderProductAndType($orderId, $productId, $orderType);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-pax-by-order2', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingPaxByOrderId2($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-notes/by-category', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingNotesByCategory($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booking-notes/by-time', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $updatedOn = $params['updated_on'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookingNotesByTime($orderId, $updatedOn);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/booking-notes/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitBookingNote($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/manage-wp-order-v3/miscellaneous-request/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitMiscellaneousRequest($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/manage-wp-order-v3/booking/update-auth-code', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $orderId = $params['order_id'] ?? '';
        $authCode = $params['auth_code'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->updateBookingAuthCode($orderId, $authCode);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/manage-wp-order-v3/azupay-dc-payment/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitAzupayDcPaymentResult($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/manage-wp-order-v3/booking/travel-date-trip-code', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTravelDateAndTripCode($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/manage-wp-order-v3/payment-request/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitPaymentRequest($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/manage-wp-order-v3/booking-by-order-id', function ($req, $res) {
    try {
        $q = $req->getQueryParams();
        $orderId = $q['order_id'] ?? '';
        if ($orderId === '') {
...
$app->get('/manage-wp-order-v3/custom-payments', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getCustomPaymentsByOrderId($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/manage-wp-order-v3/ssr-request/internal/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitInternalSsrRequest($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/manage-wp-order-v3/ssr-request/submit', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitSsrRequest($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/manage-wp-order-v3/ssr-contact-details', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSsrContactDetails($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/manage-wp-order-v3/portal-requests-by-type', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $ref = $params['reservation_ref'] ?? '';
        $type = $params['case_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPortalRequestsByType($ref, $type);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/manage-wp-order-v3/user-portal-post-meta', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $caseId = $params['case_id'] ?? '';
        $metaKey = $params['meta_key'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getUserPortalPostMeta($caseId, $metaKey);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/manage-wp-order-v3/todo-tasks', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTodoTasksByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/portal-requests', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $pnrs = isset($params['pnrs']) ? explode(',', $params['pnrs']) : [];
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getPortalRequests($orderId, $pnrs);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
...
$app->get('/v1/manage-wp-order-v3/unique-pnrs', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getUniquePnrsByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/custom-payment-status', function (Request $request, Response $response) {
    try {
        $caseId = $request->getQueryParams()['case_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getCustomPaymentByCaseId($caseId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/refunds', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getRefundsByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/v1/manage-wp-order-v3/attachment-update', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->handleAttachmentUpdate($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/update-total-amount', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->updateOrderTotalAmount($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/payment-history-filtered2', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $payTypes = isset($params['pay_types']) ? explode(',', $params['pay_types']) : [];
...
$app->get('/v1/manage-wp-order-v3/bank-account', function (Request $request, Response $response) {
    try {
        $bankId = $request->getQueryParams()['bank_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getBankAccount($bankId);
...
$app->get('/v1/manage-wp-order-v3/booking/smart-ssr-check', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $orderType = $params['order_type'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->checkSmartSsrAvailability($orderId, $orderType);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/history/meta-change-count', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getMetaChangeCount($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/v1/manage-wp-order-v3/amadeus/manual-log', function (Request $request, Response $response) {
    try {
        $params = $request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->logAmadeusManualTrigger($params);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
// 2026-01-19
$app->get('/v1/manage-wp-order-v3/payment/total-paid', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getTotalPaidForBooking($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/pax/any-pnr', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getAnyPnrByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->post('/v1/manage-wp-order-v3/payment/invoice', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->createPaymentInvoice($params);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/sms-history', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSmsHistoryByOrderId($orderId);
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/sms-templates', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ManageWpOrderV3Service();
        $data = $service->getSmsTemplates();
        return jsonResponse($response, $data);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/manage-wp-order-v3/custom-payments', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getCustomPaymentsByOrderId($orderId);
...
$app->get('/v1/manage-wp-order-v3/custom-payment-by-id', function (Request $request, Response $response) {
    try {
        $autoId = $request->getQueryParams()['auto_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getCustomPaymentByAutoId($autoId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/custom-payment/update-resend', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->updateCustomPaymentResend($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/schedule-change-history', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getScheduleChangeHistory($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/schedule-change/submit', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitScheduleChange($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/ticketing-pax/update', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->updateTicketingPax($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/ticketing-history-g360', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getTicketingHistoryG360($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/coupon/check-and-clean', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $orderId = $params['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->checkAndCleanAppliedCoupon($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/active-sales-ids', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getActiveSalesIds();
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/sale-id-update/submit', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitSaleIdUpdate($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/booked-tst-info', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getBookedTstInfo($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/issue-logs', function (Request $request, Response $response) {
    try {
        $orderId = $request->getQueryParams()['order_id'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getIssueLogsByOrderId($orderId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/issue-log/submit', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitIssueLog($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/payment-status/update', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->processPaymentStatusUpdate($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/coupon/apply', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->executeApplyCoupon($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/amadeus-name-update/log', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->logAmadeusNameUpdate($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/pricing-info', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $tripCode = $params['trip_code'] ?? '';
        $travelDate = $params['travel_date'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->getPricingInfo($tripCode, $travelDate);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/date-change/submit', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->submitDateChangeRequest($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/date-change/execute', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->executeDateChange($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/sms-history/record', function (Request $request, Response $response) {
    try {
        $params = (array)$request->getParsedBody();
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->recordSmsHistory($params);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/manage-wp-order-v3/process-return-fix', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->processReturnFix();
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/manage-wp-order-v3/ip-check', function (Request $request, Response $response) {
    try {
        $ip = $request->getQueryParams()['ip'] ?? '';
        $service = new \App\Services\ManageWpOrderV3Service();
        $result = $service->checkIpAddress($ip);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/client-base-manager2/ip-check', function (Request $request, Response $response) {
    try {
        $ip = $request->getQueryParams()['ip'] ?? '';
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->checkIpAddress($ip);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
...
$app->get('/v1/client-base-manager2/updated-balances', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->getUpdatedClientBalances();
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/client-base-manager2/search', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $search = $params['search'] ?? '';
        $options = $params['options'] ?? '';
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->searchClientBalances($search, $options);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/client-base-manager2/bulk-update', function (Request $request, Response $response) {
    try {
        $data = $request->getParsedBody();
        $updates = $data['updates'] ?? [];
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->bulkUpdateClientBalances($updates);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/client-base-manager2/filtered-balances', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $search = $params['search'] ?? '';
        $options = $params['options'] ?? '';
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->getFilteredClientBalances($search, $options);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/client-base-manager2/remark-summary', function (Request $request, Response $response) {
    try {
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->getRemarkSummary();
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->get('/v1/client-base-manager2/client-by-id', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $clientId = $params['client_id'] ?? '';
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->getClientById($clientId);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
$app->post('/v1/client-base-manager2/import', function (Request $request, Response $response) {
    try {
        $data = $request->getParsedBody();
        $clients = $data['clients'] ?? [];
        $updatedOn = $data['updated_on'] ?? date('Y-m-d H:i:s');
        $updatedBy = $data['updated_by'] ?? '';
        $service = new \App\Services\ClientBaseManageService2();
        $result = $service->importClientBalances($clients, $updatedOn, $updatedBy);
        return jsonResponse($response, $result);
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 500);
    }
...
// 2026-02-09 
$app->get('/v1/customer-view-bookings2/search', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $email = $params['email'] ?? '';
        $id = $params['id'] ?? '';
        ...
        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->searchBookings2($email, $id);

        return jsonResponse($response, $data, 'success', 'Bookings retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/pax-contact', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $column = $params['column'] ?? '';
        ...
        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getAnyPaxContact2($orderId, $column, $metaKey);

        return jsonResponse($response, ['contact' => $data], 'success', 'Pax contact retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/booking', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getBookingByOrderId2($orderId);

        return jsonResponse($response, $data, 'success', 'Booking retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
...
$app->get('/v1/customer-view-bookings2/summary', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getSummaryData2($orderId);

        return jsonResponse($response, $data, 'success', 'Summary data retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/gds-summary', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getGdsSummaryData2($orderId);

        return jsonResponse($response, $data, 'success', 'GDS Summary data retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/bookings-with-pax', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getBookingsWithPax2($orderId);

        return jsonResponse($response, $data, 'success', 'Bookings with pax retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/payment-data', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getPaymentData2($orderId);

        return jsonResponse($response, $data, 'success', 'Payment data retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/payment-history', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getPaymentHistory2($orderId);
...
$app->get('/v1/customer-view-bookings2/itinerary-bookings', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getItineraryBookings2($orderId);

        return jsonResponse($response, $data, 'success', 'Itinerary bookings retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/call-out-remark', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getLatestCallOutRemark2($orderId);

        return jsonResponse($response, ['remark' => $data], 'success', 'Call out remark retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/payment-attachments', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';
        $coOrderId = $params['co_order_id'] ?? '';
        ...
        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getPaymentAttachments2($orderId, $coOrderId, $productId);

        return jsonResponse($response, $data, 'success', 'Payment attachments retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});

$app->get('/v1/customer-view-bookings2/portal-requests', function (Request $request, Response $response) {
    try {
        $params = $request->getQueryParams();
        $orderId = $params['order_id'] ?? '';

        $service = new \App\Services\CustomerViewBookingsService();
        $data = $service->getPortalRequests2($orderId);

        return jsonResponse($response, $data, 'success', 'Portal requests retrieved');
    } catch (\Throwable $e) {
        return errorResponse($response, $e->getMessage(), 400);
    }
});
