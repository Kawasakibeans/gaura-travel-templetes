...
/**
 * Data Access Layer for Incentive-related operations
 */
class IncentiveDAL extends BaseDAL
{
    /**
    ...
    public function getDailyPerformance($fromDate, $toDate)
    {
        $sql = "
            ...
            AND combined.date BETWEEN :from_date AND :to_date
    ...
    /**
     * Get agent-wise data for a specific date (for eligibility check)
     * 
    ...
    public function getAgentDataByDate($date)
    {
        $sql = "
            ...
            FROM (
                ...
                WHERE b.order_date = :date2
            ) AS data
            JOIN wpk4_backend_agent_codes ac 
                ON BINARY data.tsr = BINARY ac.tsr
                AND BINARY ac.status = 'active'
                AND BINARY ac.team_name <> 'Others'
            LEFT JOIN (
                SELECT 
                    ai.call_date, 
                    ai.agent_name, 
                ...
                WHERE ai.call_date = :date3
                GROUP BY ai.call_date, ai.agent_name
            ) s 
                ON s.call_date = data.call_date 
                AND BINARY s.agent_name = BINARY data.agent_name
            LEFT JOIN (
                SELECT
                    ac.agent_name,
                    nd.call_date,
                ...
                JOIN wpk4_backend_agent_codes ac 
                    ON BINARY nd.tsr = BINARY ac.tsr
                WHERE nd.call_date = :date4
                GROUP BY ac.agent_name, nd.call_date
            ) nl 
                ON BINARY nl.agent_name = BINARY data.agent_name
                AND nl.call_date = data.call_date
            GROUP BY date, data.agent_name
            HAVING pif > 0 AND on_time = 'On Time' AND fcs >= 0.25 AND AHT <= 1440 AND pif_percent > 0.4
            ORDER BY date ASC
        ";
