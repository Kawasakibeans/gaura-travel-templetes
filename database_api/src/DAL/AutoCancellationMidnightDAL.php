...
class AutoCancellationMidnightDAL extends BaseDAL
{
    /**
     * Get bookings for full payment deadline cancellation (FIT & GDeals)
     */
    public function getBookingsForFullPaymentCancellation(): array
    {
        $sql = "
            SELECT 
                ...
                MIN(bookings.full_payment_deadline) AS full_payment_deadline,
            ...
            WHERE 
                ...
                AND bookings.full_payment_deadline <= NOW() 
                AND bookings.source <> 'import'
                AND bookings.order_type IN ('gds', 'WPT')
                AND bookings.total_amount <> (
                    SELECT COALESCE(SUM(p.trams_received_amount), 0.00)
                    FROM wpk4_backend_travel_payment_history p
                    WHERE p.order_id = bookings.order_id
                )
            GROUP BY 
                bookings.order_id, bookings.total_amount
            ORDER BY 
    ...
    /**
     * Check if booking was already stock changed
     */
    public function checkAlreadyStockChanged(string $orderId): bool
    {
        $sql = "
            SELECT auto_id 
            FROM wpk4_backend_travel_bookings 
            WHERE order_id = ? 
                AND payment_modified_by = 'cancel_duplicate_in_checkout'
        ";
