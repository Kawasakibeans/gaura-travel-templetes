<?php
/**
 * Knowvember Incentive Data Access Layer
 */

namespace App\DAL;

class KnowvemberIncentiveDAL extends BaseDAL
{
    /**
     * Fetch agent-level performance metrics for a single date
     */
...
HAVING 
    SUM(data.pif) > 0
    AND on_time = 'On Time'
    AND ROUND(IFNULL(SUM(data.new_sale_made_count) / NULLIF(SUM(data.gtib_count), 0), 0), 2) >= 0.30
    AND ROUND(IFNULL(SUM(data.rec_duration) / NULLIF(SUM(data.gtib_count), 0), 0), 2) <= 1440
    AND ROUND(IFNULL(SUM(data.pif) / NULLIF(SUM(data.gtib_count), 0), 0), 2) > 0.5
    AND (SUM(COALESCE(g.compliant_count, 0)) / NULLIF(SUM(COALESCE(g.audited_call, 0)), 0)) > 0.8
ORDER BY date ASC
SQL;
    ...
    /**
     * Fetch aggregated daily performance for a date range
     */
