<?php
/**
 * Knowvember Incentive Service Layer
 */

namespace App\Services;

use App\DAL\KnowvemberIncentiveDAL;
use Exception;

class KnowvemberIncentiveService
{
    private $dal;

    public function __construct()
    {
        $this->dal = new KnowvemberIncentiveDAL();
    }

    /**
     * Retrieve agent metrics for a specific date
     */
    public function getAgentMetrics(array $filters = []): array
    {
        ...
        $rows = $this->dal->getAgentMetricsByDate($date);

        $agents = array_map(function (array $row) {
            $garlandRatio = $row['garland_ratio'] !== null ? (float)$row['garland_ratio'] : null;
            ...
            return [
                ...
                'garland_ratio' => $garlandRatio,
                'garland_percent' => $garlandRatio !== null ? round($garlandRatio * 100, 2) : null,
        ...
        return [
            ...
            'thresholds' => [
                'minimum_pif' => 0,
                'minimum_fcs' => 0.30,
                'minimum_pif_percent' => 0.5,
                'maximum_aht_seconds' => 1440,
                'minimum_garland_ratio' => 0.8,
    ...
    /**
     * Retrieve aggregated daily performance for a date range
     */
    public function getDailyPerformance(array $filters = []): array
    {
        $startInput = $filters['start_date'] ?? '2025-11-01';
        $endInput = $filters['end_date'] ?? '2025-11-30';
        ...
        $rows = $this->dal->getDailyPerformance($startDate, $endDate);
        ...
        return [
            ...
            'totals' => $this->calculateDailySummary($daily),
            'daily' => $daily,
    ...
    /**
     * Build aggregate totals for daily data
     */
    private function calculateDailySummary(array $daily): array
    {
        if (empty($daily)) {
            return [
                'total_pax' => 0,
